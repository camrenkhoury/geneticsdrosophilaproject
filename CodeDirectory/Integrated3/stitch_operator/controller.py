...
class WorkflowController:
    ...
    def _set_sexing_state(self, payload: Dict[str, Any]) -> None:
        sexing_state = SexingState(
            ...
            occupancy_score=float(payload.get("occupancy_score", 0.0) or 0.0),
            occupancy_detail=str(payload.get("occupancy_detail", "") or ""),
        )
        with self._lock:
    ...
    def _sex_result_label(self, payload: Dict[str, Any]) -> str:
        return str(payload.get("label", "UNCERTAIN") or "UNCERTAIN").strip().lower()

    def _sex_result_count(self, payload: Dict[str, Any]) -> int:
        if "count" in payload:
            try:
                return max(0, int(payload.get("count", 0) or 0))
            except Exception:
                pass
        errors = [str(item) for item in payload.get("errors", [])]
        for entry in errors:
            if entry.startswith("MULTIPLE_FLIES:"):
                try:
                    return max(2, int(entry.split(":", 1)[1]))
                except Exception:
                    return 2
        if "CHAMBER_EMPTY" in errors or "NO_FLY_DETECTED" in errors:
            return 0
        if "label" in payload:
            label = self._sex_result_label(payload)
            if label in {"male", "female", "uncertain"}:
                return 1
        return 0

    def _sex_result_errors(self, payload: Dict[str, Any]) -> List[str]:
        return [str(item) for item in payload.get("errors", [])]

    def _sex_result_has_specimen(self, payload: Dict[str, Any]) -> bool:
        return self._sex_result_count(payload) > 0 or bool(payload.get("occupied", False))

    def _sex_result_has_routable_specimen(self, payload: Dict[str, Any]) -> bool:
        return self._sex_result_count(payload) > 0

    def _sex_result_is_confirmed_candidate(self, payload: Dict[str, Any]) -> bool:
        label = self._sex_result_label(payload)
        return self._sex_result_count(payload) == 1 and label in {"male", "female"}

    def _format_sexing_pass_detail(self, payload: Dict[str, Any], *, pass_index: int) -> str:
        label = self._sex_result_label(payload).upper()
        count = self._sex_result_count(payload)
        confidence = float(payload.get("confidence", 0.0) or 0.0)
        errors = self._sex_result_errors(payload)
        base_detail = str(payload.get("detail", "") or "").strip()
        bits = [f"Pass {pass_index}: count={count}", f"label={label}", f"conf={confidence:.2f}"]
        if errors:
            bits.append("rejects=" + ", ".join(errors))
        if base_detail:
            bits.append(base_detail)
        return " | ".join(bits)

    def _merge_sexing_payloads(
        self,
        ...
        *,
        ...
        force_uncertain: Optional[bool] = None,
        force_confidence: Optional[float] = None,
    ) -> Dict[str, Any]:
        payloads = [primary]
        if secondary is not None:
            payloads.append(secondary)
        return self._merge_sexing_sequence(
            payloads,
            detail=detail,
    ...
    def _merge_sexing_sequence(
        ...
        *,
        ...
        sequence = [dict(item) for item in payloads if item is not None]
        merged = dict(sequence[-1]) if sequence else {}
        errors: List[str] = []
        for source in sequence:
            for item in source.get("errors", []):
                ...
                if entry and entry not in errors:
                    errors.append(entry)
        for item in extra_errors or []:
            entry = str(item)
            if entry and entry not in errors:
                errors.append(entry)
        merged["errors"] = errors
        merged["detail"] = str(detail or "")
        if force_label is not None:
            merged["label"] = str(force_label)
        if force_uncertain is not None:
            merged["uncertain"] = bool(force_uncertain)
        if force_confidence is not None:
            merged["confidence"] = float(force_confidence)
        if "count" not in merged:
            merged["count"] = self._sex_result_count(sequence[-1] if sequence else {})
        if sequence:
            merged["verification_passes_completed"] = len(sequence)
        return merged

    def _requested_verification_passes(self) -> int:
        try:
            requested = int(float(getattr(self.settings, "sexing_verification_passes", 3) or 3))
        except Exception:
            requested = 3
        return max(1, min(requested, 3))

    def _verification_majority_threshold(self, requested_passes: Optional[int] = None) -> int:
        passes = self._requested_verification_passes() if requested_passes is None else max(1, min(int(requested_passes), 3))
        if passes <= 1:
            return 1
        return (passes // 2) + 1

    def _resolve_verified_sexing_result(
        self,
        *,
        fly_index: int,
        first_pass: Dict[str, Any],
    ) -> Dict[str, Any]:
        requested_passes = self._requested_verification_passes()
        threshold = self._verification_majority_threshold(requested_passes)
        passes: List[Dict[str, Any]] = [dict(first_pass)]

        for pass_index in range(2, requested_passes + 1):
            if not self._sex_result_has_routable_specimen(passes[-1]):
                break
            self._redrop_for_resex(
                fly_index=fly_index,
                next_pass_index=pass_index,
                total_passes=requested_passes,
            )
            passes.append(self._classify_sexing_pass(fly_index=fly_index, pass_index=pass_index))

        male_votes = [payload for payload in passes if self._sex_result_count(payload) == 1 and self._sex_result_label(payload) == "male"]
        female_votes = [payload for payload in passes if self._sex_result_count(payload) == 1 and self._sex_result_label(payload) == "female"]
        rejected_votes = len(passes) - len(male_votes) - len(female_votes)
        completed_passes = len(passes)
        pass_details = [str(payload.get("detail", "") or "").strip() for payload in passes if str(payload.get("detail", "") or "").strip()]
        last_pass = passes[-1]
        last_count = self._sex_result_count(last_pass)
        last_label = self._sex_result_label(last_pass)
        vote_summary = f"votes male={len(male_votes)}, female={len(female_votes)}, rejected={rejected_votes}"
        all_uncertain_singletons = bool(passes) and all(
            self._sex_result_count(payload) == 1 and self._sex_result_label(payload) not in {"male", "female"}
            for payload in passes
        )

        majority_label = ""
        majority_votes: List[Dict[str, Any]] = []
        if len(male_votes) >= threshold and len(male_votes) >= len(female_votes):
            majority_label = "male"
            majority_votes = male_votes
        elif len(female_votes) >= threshold and len(female_votes) > len(male_votes):
            majority_label = "female"
            majority_votes = female_votes

        if majority_label and last_count == 1:
            destination, destination_mode = self.choose_auto_destination(majority_label)
            route_from_chamber = True
            outcome = "routed"
            if requested_passes <= 1:
                summary = f"Single-pass classification returned {majority_label.upper()}."
            elif requested_passes == 2:
                summary = f"Verified {majority_label.upper()} on both passes."
            else:
                summary = f"Majority vote {majority_label.upper()} ({len(majority_votes)}/{completed_passes} passes)."
            if destination_mode == "junk":
                summary = (
                    f"{summary} The {majority_label.upper()} vials are full, so the specimen is being sent to {destination.label}."
                )
            supporting_confidences = [
                float(payload.get("confidence", 0.0) or 0.0)
                for payload in majority_votes
            ]
            final_result = self._merge_sexing_sequence(
                passes,
                detail=" || ".join(pass_details + [summary]),
                force_label=majority_label,
                force_uncertain=False,
                force_confidence=min(supporting_confidences) if supporting_confidences else 0.0,
            )
            final_result["verification_passes_requested"] = requested_passes
            final_result["verification_vote_summary"] = vote_summary
            return {
                "passes": passes,
                "final_result": final_result,
                "destination": destination,
                "destination_mode": destination_mode,
                "route_from_chamber": route_from_chamber,
                "outcome": outcome,
                "reject_reason": "",
                "all_uncertain_singletons": all_uncertain_singletons,
            }

        destination = self.choose_junk_destination()
        destination_mode = "junk"
        route_from_chamber = self._sex_result_has_routable_specimen(last_pass)
        outcome = "sexing_rejected"

        if requested_passes <= 1:
            if last_count <= 0:
                reject_reason = f"Rejected after first sex pass because count={last_count}"
            elif last_count > 1:
                reject_reason = f"Rejected after first sex pass because multiple flies were detected (count={last_count})"
            else:
                reject_reason = f"Rejected after first sex pass because the classifier returned {last_label.upper()}"
        elif majority_label and last_count <= 0:
            reject_reason = (
                f"Verification reached a {majority_label.upper()} majority, but the final pass did not isolate a fly"
            )
        elif majority_label and last_count > 1:
            reject_reason = (
                f"Verification reached a {majority_label.upper()} majority, but the final pass detected multiple flies (count={last_count})"
            )
        elif completed_passes < requested_passes and last_count <= 0:
            reject_reason = (
                f"Verification ended early because pass {completed_passes} did not isolate a fly ({vote_summary})"
            )
        elif last_count > 1:
            reject_reason = f"Final sex pass found multiple flies (count={last_count}; {vote_summary})"
        elif len(male_votes) > 0 and len(female_votes) > 0:
            reject_reason = f"No majority sex classification ({vote_summary})"
        elif all_uncertain_singletons:
            reject_reason = f"All sexing passes were uncertain ({vote_summary})"
        else:
            reject_reason = f"Sex verification failed ({vote_summary})"

        extra_errors = ["SEXING_VERIFICATION_FAILED"]
        if majority_label == "":
            extra_errors.append("REJECT_NO_MAJORITY")
        if last_count <= 0:
            extra_errors.append("REJECT_COUNT_0")
        elif last_count > 1:
            extra_errors.append(f"REJECT_COUNT_{last_count}")
        elif last_label not in {"male", "female"}:
            extra_errors.append("REJECT_UNCERTAIN")
        if len(male_votes) > 0 and len(female_votes) > 0:
            extra_errors.append(f"SEX_MIXED_VOTES:M{len(male_votes)}_F{len(female_votes)}")

        summary = (
            f"{reject_reason}. "
            + (
                f"Sending the specimen to {destination.label}."
                if route_from_chamber
                else "No specimen remained in the chamber; verifying against the channel."
            )
        )
        final_result = self._merge_sexing_sequence(
            passes,
    ...
    def _classify_sexing_pass(self, *, fly_index: int, pass_index: int) -> Dict[str, Any]:
        time.sleep(max(0.0, float(self.settings.classification_delay_s)))
        result = dict(self.sexing.classify())
        result["detail"] = self._format_sexing_pass_detail(result, pass_index=pass_index)
        self._set_sexing_state(result)
        self.log(f"Fly {fly_index}: {result['detail']}")
        return result

    def _redrop_for_resex(self, *, fly_index: int, next_pass_index: int = 2, total_passes: Optional[int] = None) -> None:
        phase_label = (
            f"pass {next_pass_index}/{total_passes}"
            if total_passes is not None and int(total_passes) > 1
            else f"pass {next_pass_index}"
        )
        self._update(
            current_target=f"Fly {fly_index}: re-sexing",
            selected_destination="",
            status_message=f"Fly {fly_index}: re-checking sex with verification {phase_label}.",
        )
        self.log(f"Fly {fly_index}: re-acquiring specimen for sex verification {phase_label}.")
        ...
        self._check_cancelled(
            "Routing stopped during sex verification. Use Debug > Manual recovery if needed."
        )

    def _route_from_chamber_to_destination(
        self,
        ...
        *,
        ...
        self.log(f"Fly {fly_index}: chamber drop -> {destination.label}.")
    ...
    def _auto_route_from_source_impl(
        ...
        *,
        ...
        first_pass = self._classify_sexing_pass(fly_index=fly_index, pass_index=1)
        verification = self._resolve_verified_sexing_result(
            fly_index=fly_index,
            first_pass=first_pass,
        )
        final_sex_result = dict(verification["final_result"])
        destination = verification["destination"]
        destination_mode = str(verification["destination_mode"])
        route_from_chamber = bool(verification["route_from_chamber"])
        performed_route = False
        ...
        reject_reason = str(verification["reject_reason"])
        all_uncertain_singletons = bool(verification.get("all_uncertain_singletons", False))

        self._set_sexing_state(final_sex_result)
        self.log(
            f"Fly {fly_index}: final sexing detail -> {str(final_sex_result.get('detail', '') or '').strip()}"
        )
        ...
        if (
            outcome == "sexing_rejected"
            and route_from_chamber
            and all_uncertain_singletons
        ):
            self._update(
                selected_destination="",
                current_target=f"Fly {fly_index}: verifying uncertain pickup",
                status_message=(
                    f"Fly {fly_index}: uncertain sexing passes. Refreshing the channel before routing to {destination.label}."
                ),
            )
            ...
            if pickup_success:
                ...
                updated_sex_result = self._merge_sexing_payloads(
                    final_sex_result,
                    detail=(
                        f"{str(final_sex_result.get('detail', '') or '').strip()} || {pickup_miss_message}"
                    ),
                    extra_errors=["PICKUP_MISS_VERIFIED"],
                    force_label="UNCERTAIN",
                    force_uncertain=True,
                    force_confidence=0.0,
                )
                self._set_sexing_state(updated_sex_result)
                chamber_inspection = self._inspect_chamber()
                chamber_count = self._sex_result_count(chamber_inspection)
                if chamber_count > 0:
                    ...
                    if not chamber_clear_success:
                        ...
                        return {
                            ...
                            "destination_mode": destination_mode,
                            "sex_label": str(updated_sex_result.get("label", "UNCERTAIN")),
                            "confidence": float(updated_sex_result.get("confidence", 0.0) or 0.0),
                            ...
                            "count": self._sex_result_count(updated_sex_result),
                            "errors": self._sex_result_errors(updated_sex_result),
                            "reject_reason": reject_reason,
                        }
                elif bool(chamber_inspection.get("occupied", False)):
                    self.log(
                        f"Fly {fly_index}: chamber looked occupied after a likely pickup miss, but count=0 so no automatic junk recovery was attempted."
                    )
        ...
        if route_from_chamber:
            route_message = (
                f"Fly {fly_index}: sending confirmed specimen to {destination.label}."
                if outcome == "routed" and destination_mode != "junk"
                else f"Fly {fly_index}: sending rejected specimen to {destination.label}."
            )
            self._route_from_chamber_to_destination(
                destination,
                fly_index=fly_index,
                status_message=route_message,
            )
            performed_route = True
        else:
            self._update(
                selected_destination="",
                current_target=f"Fly {fly_index}: verifying reject",
                status_message=(
                    f"Fly {fly_index}: no specimen remained in the chamber. "
                    "Refreshing the channel to verify the source location."
                ),
            )

        if verified_channel_result is not None:
        ...
        chamber_destination = destination if performed_route else self.choose_junk_destination()
        chamber_inspection = self._inspect_chamber()
        chamber_count = self._sex_result_count(chamber_inspection)
        if chamber_count > 0:
            clear_info = self._attempt_chamber_clear(
                destination=chamber_destination,
            ...
            if not chamber_clear_success:
                blocked_message = (
                    f"Fly {fly_index}: the sexing chamber still appears occupied after routing. "
                    "Use Debug > Manual recovery before continuing."
                )
                updated_sex_result = self._merge_sexing_payloads(
                    final_sex_result,
                    detail=(
                        f"{str(final_sex_result.get('detail', '') or '').strip()} || {blocked_message}"
                    ),
                    extra_errors=["CHAMBER_BLOCKED"],
                    force_label="UNCERTAIN",
                    force_uncertain=True,
                )
                self._set_sexing_state(updated_sex_result)
                self._update(current_target="", selected_destination=destination.label if performed_route else "", status_message=blocked_message)
                self.log(blocked_message)
                return {
                    "outcome": "chamber_blocked",
                    "channel_result": after_result,
            ...
            self.log(
                f"Fly {fly_index}: chamber looked occupied after routing, but count=0 so no automatic junk recovery was attempted."
            )
        ...
        if outcome == "routed":
            ...
            if destination_mode == "junk":
                message = (
                    f"Fly {fly_index}: classified as {self._sex_result_label(final_sex_result).upper()} after verification, "
                    f"but sex-specific vials are full; sent to {destination.label}. {after_count} flies remain."
                )
            else:
                message = (
                    f"Fly {fly_index}: classified as {self._sex_result_label(final_sex_result).upper()} after verification; "
                    f"sent to {destination.label}. {after_count} flies remain in the channel."
                )
            ...
            return {
                ...
                "destination_label": destination.label,
                "destination_mode": destination_mode,
                "sex_label": str(final_sex_result.get("label", "UNCERTAIN")),
                "confidence": float(final_sex_result.get("confidence", 0.0) or 0.0),
                "after_count": after_count,
                "source_still_present": False,
                "chamber_clear_attempts": chamber_clear_attempts,
                "chamber_clear_success": chamber_clear_success,
                "status_message": message,
                "count": self._sex_result_count(final_sex_result),
                "errors": self._sex_result_errors(final_sex_result),
                "reject_reason": reject_reason,
            }

        if performed_route and destination_mode == "junk":
            self.increment_vial(destination.vial_id)
        if performed_route:
            message = (
                f"Fly {fly_index}: sexing rejected; sent to {destination.label}. {after_count} flies remain in the channel."
            )
        else:
            message = (
                f"Fly {fly_index}: sexing rejected with no specimen left in the chamber. "
                f"{after_count} flies remain in the channel."
            )
        if reject_reason:
            message = f"{message} {reject_reason}."
        self._update(current_target="", selected_destination=destination.label if performed_route else "", status_message=message)
        self.log(message)
        return {
            "outcome": "sexing_rejected",
            "channel_result": after_result,
    ...
    def start_auto_flow(self) -> bool:
        def _auto_stats_message(
            *,
            ...
            total_rejects = int(pickup_rejects) + int(sexing_rejects)
            return (
                f"Auto stats: routed {routed_count} · rejects {total_rejects} "
                f"(pickup {pickup_rejects}, sexing {sexing_rejects}) · skipped {skipped_count}."
            )
        ...
        def task():
            ...
            preflight_count = self._sex_result_count(preflight_inspection)
            ...
            elif bool(preflight_inspection.get("occupied", False)):
                self.log(
                    "Sexing chamber preflight was ambiguous (occupied flag with count=0). Continuing without automatic recovery."
                )
            ...
            with capture_session:
                ...
                master_payload: Dict[str, Any] = {
                    ...
                    "sexing_verification_passes": self._requested_verification_passes(),
                }
                ...
                def _write_master() -> None:
                    ...
                    master_payload["stats"] = {
                        ...
                        "sexing_rejects": sexing_rejects,
                        "total_rejects": pickup_rejects + sexing_rejects,
                ...
                while True:
                    ...
                    if source_position is None:
                        ...
                        message = (
                            ...
                            f"{_auto_stats_message(routed_count=routed_count, pickup_rejects=pickup_rejects, sexing_rejects=sexing_rejects, skipped_count=len(skipped_positions_mm))}"
                        )
                        self._set_stage(
                            WorkflowStage.READY,
                            "Auto flow paused",
                    ...
                    history_entry = {
                        ...
                        "count": int(attempt.get("count", 0) or 0),
                        "errors": [str(item) for item in attempt.get("errors", [])],
                        "reject_reason": str(attempt.get("reject_reason", "") or ""),
                    ...
                    combined_message = (
                        f"{status_message} {_auto_stats_message(routed_count=routed_count, pickup_rejects=pickup_rejects, sexing_rejects=sexing_rejects, skipped_count=len(skipped_positions_mm))}"
                        if status_message
                        else _auto_stats_message(routed_count=routed_count, pickup_rejects=pickup_rejects, sexing_rejects=sexing_rejects, skipped_count=len(skipped_positions_mm))
                    )
    ...
    def inspect_chamber_debug(self) -> Dict[str, Any]:
        payload = dict(self.sexing.inspect_chamber(debug=True) or {})
        count = int(payload.get("count", 0) or 0)
        detail = str(payload.get("detail", "") or "").strip()
        errors = [str(item) for item in payload.get("errors", [])]
        self._set_sexing_state({
            "captured_at": payload.get("captured_at", ""),
            "label": "UNCERTAIN" if count != 1 else "--",
            "confidence": 0.0,
            "image_path": payload.get("debug_image_path", "") or payload.get("image_path", ""),
            "detail": detail or (f"Debug chamber inspection count={count}."),
            "uncertain": count != 1,
            "count": count,
            "errors": errors,
            "debug_image_path": payload.get("debug_image_path", ""),
            "occupancy_score": payload.get("occupancy_score", 0.0),
            "occupancy_detail": payload.get("occupancy_detail", ""),
        })
        self.log(f"Debug chamber inspection: {detail or f'count={count}'}")
        return payload
