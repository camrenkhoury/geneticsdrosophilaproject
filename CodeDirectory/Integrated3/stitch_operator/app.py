...
from .assay_embed import EmbeddedAssayUI
...
try:
    import numpy as np
except Exception:  # pragma: no cover - numpy should exist on the Pi, but keep a safe fallback
    np = None
...
class ScrollableFrame(tk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        # Mouse wheel scrolling is needed on the Pi/touchpad setup because many
        # pages are taller than the available viewport.
    ...
    def _can_scroll(self) -> bool:
        try:
            ...
            if not bbox:
                return False
            return (bbox[3] - bbox[1]) > max(0, self.canvas.winfo_height() - 4)
        except Exception:
            return False

    def _scroll_units(self, units: int):
        if not self.winfo_ismapped() or not self._pointer_within() or not self._can_scroll():
            return None
...
class OperatorApp(tk.Tk):
    def __init__(self):
        ...
        default_w = min(1640, max(1120, screen_w - 48))
        default_h = min(1020, max(760, screen_h - 96))
        self.geometry(f"{default_w}x{default_h}")
        self.minsize(min(default_w, 1120), min(default_h, 720))
        ...
        self._advanced_unlock_expires_at = 0.0
        self._embedded_assay_ui: Optional[EmbeddedAssayUI] = None
        self._embedded_assay_assay_page: Optional[tk.Frame] = None
        self._embedded_assay_debug_page: Optional[tk.Frame] = None
        ...
        self.tuning_verification_passes_var = tk.StringVar(value="3")
        ...
        self.error_corner_sample_var = tk.StringVar(value="")
        self.error_bg_tolerance_var = tk.StringVar(value="")
        self.error_open_kernel_var = tk.StringVar(value="")
        self.error_close_kernel_var = tk.StringVar(value="")
        self.error_erode_kernel_var = tk.StringVar(value="")
        self.error_erode_iterations_var = tk.StringVar(value="")
        self.error_min_area_frac_var = tk.StringVar(value="")
        self.error_max_area_px_var = tk.StringVar(value="")
        ...
        self._reload_error_tuning_vars_from_controller()
    ...
    def _build_shell(self):
        ...
        header = tk.Frame(self, bg=PALETTE.surface_highest, height=64)
        ...
        nav.grid(row=1, column=0, sticky="ew", padx=18, pady=(10, 0))
        ...
        footer = tk.Frame(self, bg=PALETTE.surface_lowest, height=46)
    ...
    def _action_button(self, parent, text: str, command, *, primary: bool = True, width: Optional[int] = 18) -> tk.Button:
        kwargs = {
            "text": text,
            "command": command,
    ...
    def _button_grid(self, parent, specs: List[Tuple[str, object, bool]], *, columns: int = 2, width: Optional[int] = None) -> tk.Frame:
        frame = tk.Frame(parent, bg=parent.cget("bg"))
        frame.pack(fill="x", pady=(12, 0))
        for column in range(columns):
            frame.columnconfigure(column, weight=1)
        for idx, (text, command, primary) in enumerate(specs):
            row = idx // columns
            column = idx % columns
            padx = (0, 8) if column == 0 else (8, 0)
            self._action_button(frame, text, command, primary=primary, width=width).grid(
                row=row,
                column=column,
                sticky="ew",
                padx=padx,
    ...
    def _make_scroll_page(self, parent) -> Tuple[ScrollableFrame, tk.Frame]:
        page = ScrollableFrame(parent, bg=PALETTE.background)
        body = page.inner
        body.configure(bg=PALETTE.background)
    ...
    def _image_panel(
        ...
        *,
        width: int = 520,
        height: int = 300,
        display_mode: str = "contain",
        trim_black: bool = False,
    ) -> tk.Frame:
    ...
    def _trim_black_borders(self, image):
        ...
        try:
            if np is not None:
                grayscale = np.asarray(image.convert("L"), dtype="uint8")
                if grayscale.ndim == 2 and grayscale.size > 0:
                    col_max = grayscale.max(axis=0)
                    col_mean = grayscale.mean(axis=0)
                    col_active = (grayscale >= 18).mean(axis=0)
                    row_max = grayscale.max(axis=1)
                    row_mean = grayscale.mean(axis=1)
                    row_active = (grayscale >= 18).mean(axis=1)

                    def _find_start(max_vals, mean_vals, active_vals):
                        idx = 0
                        limit = len(max_vals) - 1
                        while idx < limit and max_vals[idx] <= 22 and mean_vals[idx] <= 6 and active_vals[idx] <= 0.01:
                            idx += 1
                        return idx

                    def _find_end(max_vals, mean_vals, active_vals):
                        idx = len(max_vals) - 1
                        while idx > 0 and max_vals[idx] <= 22 and mean_vals[idx] <= 6 and active_vals[idx] <= 0.01:
                            idx -= 1
                        return idx

                    left = _find_start(col_max, col_mean, col_active)
                    right = _find_end(col_max, col_mean, col_active)
                    top = _find_start(row_max, row_mean, row_active)
                    bottom = _find_end(row_max, row_mean, row_active)

                    width = max(1, int(right - left + 1))
                    height = max(1, int(bottom - top + 1))
                    if width >= max(40, int(image.width * 0.10)) and height >= max(24, int(image.height * 0.08)):
                        if left > 1 or top > 1 or right < image.width - 2 or bottom < image.height - 2:
                            pad = 14
                            return image.crop((
                                max(0, int(left) - pad),
                                max(0, int(top) - pad),
                                min(image.width, int(right) + 1 + pad),
                                min(image.height, int(bottom) + 1 + pad),
                            ))

            grayscale = image.convert("L")
            mask = grayscale.point(lambda px: 255 if int(px) > 6 else 0)
            bbox = mask.getbbox()
            if not bbox:
                return image
    ...
    def _update_image_widget(self, key: str, path_text: str, placeholder: str) -> None:
        ...
        try:
            ...
            if bool(target.get("trim_black")):
                image = self._trim_black_borders(image)
            src_w, src_h = image.size
            if src_w <= 0 or src_h <= 0:
                raise ValueError("Invalid image size")
            display_mode = str(target.get("display_mode", "contain") or "contain").lower()
            ...
            if display_mode == "cover":
                ...
                working = image.resize((scaled_w, scaled_h), resample) if (scaled_w, scaled_h) != image.size else image
                crop_left = max(0, (scaled_w - box_w) // 2)
                crop_top = max(0, (scaled_h - box_h) // 2)
                canvas = working.crop((crop_left, crop_top, crop_left + box_w, crop_top + box_h))
            else:
    ...
    def _show_page(self, name: str):
        if name == "Debug / Advanced" and not self._require_advanced_unlock():
            return
        self._active_page = name
        if name in {"Assay", "Results"} and self._embedded_assay_ui is not None:
            try:
        ...
        for page_name, page in self._pages.items():
            if page_name == name:
                page.tkraise()
                if isinstance(page, ScrollableFrame):
    ...
    def _require_advanced_unlock(self) -> bool:
        if time.monotonic() < float(self._advanced_unlock_expires_at):
            return True
        value = simpledialog.askstring(
            "Protected controls",
            "Enter the 4-digit code to open Debug / Advanced.",
            parent=self,
            show="*",
        )
        if value is None:
            return False
        if str(value).strip() != "1234":
            messagebox.showerror("Incorrect code", "Protected controls remain locked.")
            return False
        self._advanced_unlock_expires_at = time.monotonic() + 15.0 * 60.0
        return True

    def _ensure_embedded_assay_ui(self) -> EmbeddedAssayUI:
        if self._embedded_assay_ui is None:
            self._embedded_assay_ui = EmbeddedAssayUI(self, controller=self.controller)
        return self._embedded_assay_ui

    def _maybe_attach_embedded_assay_pages(self) -> None:
        if self._embedded_assay_attached:
            return
        if self._embedded_assay_assay_page is None or self._embedded_assay_debug_page is None:
            return
        ui = self._ensure_embedded_assay_ui()
        ui.attach_pages(self._embedded_assay_assay_page, self._embedded_assay_debug_page)
    ...
    def _build_workflow_page(self, parent) -> tk.Frame:
        page, body = self._make_scroll_page(parent)
        ...
        top_card, top_body = self._card(body, "Auto loading")
        ...
        self._action_button(controls, "START AUTO", lambda: self.controller.start_auto_flow(), width=18).pack(fill="x", pady=(10, 8))
        self._action_button(controls, "STOP", lambda: self.controller.stop_current_task(), primary=False, width=18).pack(fill="x")
        self._action_button(controls, "RESET COUNTS", self._confirm_reset_vial_counts, primary=False, width=18).pack(fill="x", pady=(12, 0))
        tk.Label(controls, text="Assay actions", bg=PALETTE.surface_lowest, fg=PALETTE.text_muted, font=LABEL_FONT).pack(anchor="w", pady=(18, 4))
        self._action_button(controls, "RUN ASSAY", lambda: self.controller.start_run_assay(), primary=False, width=18).pack(fill="x", pady=(6, 8))
        self._action_button(controls, "PROCESS ASSAY", lambda: self.controller.start_process_last_assay(), primary=False, width=18).pack(fill="x", pady=8)
        self._action_button(controls, "EXPORT REPORT TO BOX", lambda: self.controller.start_upload_last_run(), primary=False, width=18).pack(fill="x", pady=(8, 0))
        ...
        vial_section, vial_body = self._card(body, "Destination vials")
        vial_section.grid(row=1, column=0, sticky="ew", pady=(0, 16))
        self._build_vial_cards(vial_body, "workflow")

        log_card, log_body = self._card(body, "Live workflow log")
    ...
    def _build_channel_page(self, parent) -> tk.Frame:
        page, body = self._make_scroll_page(parent)
        content = tk.Frame(body, bg=PALETTE.background)
        ...
        right = tk.Frame(content, bg=PALETTE.background)
        ...
        self._button_grid(
            setup_body,
            [
                ("Capture Background", lambda: self.controller.start_capture_channel_background(), True),
                ("Calibrate", lambda: self.controller.start_calibrate_channel(), False),
                ("Capture Channel", lambda: self.controller.start_capture_channel(), True),
            ],
            columns=2,
            width=None,
        )
    ...
    def _build_loading_page(self, parent) -> tk.Frame:
        ...
        left = tk.Frame(content, bg=PALETTE.background)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        self._image_panel(left, "loading_sexing", "Sexing chamber", width=1040, height=620).pack(fill="both", expand=True)
    ...
    def _build_assay_page(self, parent) -> tk.Frame:
        ...
        self._embedded_assay_assay_page = page
        self._maybe_attach_embedded_assay_pages()
        return page
    def _build_results_page(self, parent) -> tk.Frame:
        ...
        self._embedded_assay_debug_page = page
        self._maybe_attach_embedded_assay_pages()
        return page
    def _build_debug_page(self, parent) -> tk.Frame:
        ...
        tuning_card, tuning_body = self._card(body, "Channel + loading tuning", "Adjust pickup offset, pickup/drop timing, release settle, classification timing, sex verification passes, channel approach position, chamber clear offset, and the channel calibration length here. Save applies immediately and persists to operator_settings.json.")
        ...
        for idx, (label, var) in enumerate([
            ...
            ("Sex verification passes (1 or 3)", self.tuning_verification_passes_var),
        ...
        error_card, error_body = self._card(
            body,
            "Sexing error detection",
            "These controls tune the blob-count error checks used before YOLO classification. Use them only when the chamber is miscounting empty, single-fly, or multi-fly cases.",
        )
        error_card.grid(row=4, column=0, sticky="ew", pady=(0, 18))
        error_grid = tk.Frame(error_body, bg=PALETTE.surface_low)
        error_grid.pack(fill="x")
        for idx, (label, var) in enumerate([
            ("Corner sample (px)", self.error_corner_sample_var),
            ("Background tolerance", self.error_bg_tolerance_var),
            ("Open kernel", self.error_open_kernel_var),
            ("Close kernel", self.error_close_kernel_var),
            ("Erode kernel", self.error_erode_kernel_var),
            ("Erode iterations", self.error_erode_iterations_var),
            ("Min fly area fraction", self.error_min_area_frac_var),
            ("Max single area (px)", self.error_max_area_px_var),
        ]):
            row = tk.Frame(error_grid, bg=PALETTE.surface_low)
            row.grid(row=idx // 2, column=idx % 2, sticky="ew", padx=(0 if idx % 2 == 0 else 10, 0), pady=6)
            tk.Label(row, text=label, bg=PALETTE.surface_low, fg=PALETTE.text_muted, font=LABEL_FONT).pack(anchor="w")
            ttk.Entry(row, textvariable=var, width=18).pack(anchor="w", pady=(4, 0))
        error_grid.columnconfigure(0, weight=1)
        error_grid.columnconfigure(1, weight=1)
        error_btns = tk.Frame(error_body, bg=PALETTE.surface_low)
        error_btns.pack(fill="x", pady=(12, 0))
        self._action_button(error_btns, "Inspect Chamber Now", lambda: self._run_debug_action(self.controller.inspect_chamber_debug), primary=False, width=18).pack(side="left", padx=(0, 8))
        self._action_button(error_btns, "Reload Detection", self._reload_error_tuning_vars_from_controller, primary=False, width=16).pack(side="left", padx=8)
        self._action_button(error_btns, "Save Detection", self._save_error_tuning_fields, primary=False, width=16).pack(side="left", padx=8)
        ...
        assay_card.grid(row=5, column=0, sticky="ew", pady=(0, 18))
        ...
        model_card.grid(row=6, column=0, sticky="ew", pady=(0, 18))
        ...
        settings_card.grid(row=7, column=0, sticky="nsew", pady=(0, 18))
        ...
        state_card.grid(row=8, column=0, sticky="nsew", pady=(0, 18))
        ...
        log_card.grid(row=9, column=0, sticky="nsew")
    ...
    def _reload_tuning_vars_from_controller(self):
        ...
        self.tuning_verification_passes_var.set(str(int(getattr(settings, 'sexing_verification_passes', 3) or 3)))
    ...
    def _reload_error_tuning_vars_from_controller(self):
        settings = self.controller.settings
        self.error_corner_sample_var.set(str(int(getattr(settings, 'sexing_error_corner_sample_px', 20))))
        self.error_bg_tolerance_var.set(str(int(getattr(settings, 'sexing_error_bg_tolerance', 65))))
        self.error_open_kernel_var.set(str(int(getattr(settings, 'sexing_error_open_kernel_size', 3))))
        self.error_close_kernel_var.set(str(int(getattr(settings, 'sexing_error_close_kernel_size', 11))))
        self.error_erode_kernel_var.set(str(int(getattr(settings, 'sexing_error_erode_kernel_size', 11))))
        self.error_erode_iterations_var.set(str(int(getattr(settings, 'sexing_error_erode_iterations', 9))))
        self.error_min_area_frac_var.set(f"{float(getattr(settings, 'sexing_error_single_fly_min_frac', 0.001)):.4f}")
        self.error_max_area_px_var.set(str(int(float(getattr(settings, 'sexing_error_single_fly_max_area_px', 40000)))))

    def _save_error_tuning_fields(self):
        try:
            patch = {
                "sexing_error_corner_sample_px": int(float(self.error_corner_sample_var.get())),
                "sexing_error_bg_tolerance": int(float(self.error_bg_tolerance_var.get())),
                "sexing_error_open_kernel_size": int(float(self.error_open_kernel_var.get())),
                "sexing_error_close_kernel_size": int(float(self.error_close_kernel_var.get())),
                "sexing_error_erode_kernel_size": int(float(self.error_erode_kernel_var.get())),
                "sexing_error_erode_iterations": int(float(self.error_erode_iterations_var.get())),
                "sexing_error_single_fly_min_frac": float(self.error_min_area_frac_var.get()),
                "sexing_error_single_fly_max_area_px": int(float(self.error_max_area_px_var.get())),
            }
        except ValueError:
            messagebox.showerror("Detection tuning error", "All error-detection fields must be numeric.")
            return
        try:
            self.controller.patch_settings_fields(**patch)
            self._reload_settings_editor_from_controller()
            self._reload_error_tuning_vars_from_controller()
            self.footer_var.set("Sexing error-detection tuning saved.")
        except Exception as exc:
            messagebox.showerror("Detection tuning error", str(exc))
    ...
    def _save_tuning_fields(self):
        try:
            patch = {
                ...
                "sexing_verification_passes": int(float(self.tuning_verification_passes_var.get())),
            ...
            return
        if int(patch["sexing_verification_passes"]) not in {1, 3}:
            messagebox.showerror("Tuning error", "Sex verification passes must be set to 1 or 3.")
            return
        try:
            self.controller.patch_settings_fields(**patch)
            self._reload_settings_editor_from_controller()
    ...
    def _refresh_status(self, snapshot: OperatorState):
        ...
        loading_lines = []
        if snapshot.sexing.label != "--" or snapshot.sexing.count or snapshot.sexing.errors:
            loading_lines.append(f"Count: {int(snapshot.sexing.count)}")
        if snapshot.sexing.errors:
            loading_lines.append("Rejects: " + ", ".join(str(item) for item in snapshot.sexing.errors[:6]))
        detail_text = str(snapshot.sexing.detail or "").strip()
        if detail_text:
            loading_lines.append(detail_text)
        self.loading_detail_var.set("\n".join(line for line in loading_lines if line).strip() or "Route the next fly to refresh the sexing result.")
