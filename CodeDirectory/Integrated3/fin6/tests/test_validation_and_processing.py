...
from assay_tracking import VialCalibration, build_assay_calibration, ensure_calibration_matches_shape, load_assay_calibration, save_assay_calibration
...
class ValidationAndProcessingTests(unittest.TestCase):
    ...
    def test_validation_auto_resizes_calibration_to_match_background(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            profile = AssayProfile(name="validation_resize")
            profile.outputs.background_root = str(root / "backgrounds")
            profile.calibration_path = str(root / "calibration.json")

            background_root = root / "backgrounds" / profile.name
            background_root.mkdir(parents=True, exist_ok=True)
            transformed_path = background_root / "current_transformed.png"
            raw_path = background_root / "current_raw.png"
            meta_path = background_root / "current_meta.json"

            background = np.full((120, 60, 3), 255, dtype=np.uint8)
            cv2.imwrite(str(transformed_path), background)
            cv2.imwrite(str(raw_path), background)
            save_json(meta_path, {"transform_signature": profile.transform.signature()})

            stale_background = np.full((240, 120, 3), 255, dtype=np.uint8)
            vial = VialCalibration(
                physical_index=1,
                assay_index=1,
                enabled=True,
                roi_xywh=[20, 20, 60, 180],
                top_point_px=[50, 20],
                baseline_point_px=[50, 219],
                threshold_point_px=[50, 120],
                tube_height_mm=100.0,
                label="Tube 1",
            )
            calibration = build_assay_calibration(stale_background, [vial])
            save_assay_calibration(profile.calibration_path, calibration)

            result = validate_recording_requirements(profile, root)
            repaired = load_assay_calibration(profile.calibration_path)
            backups = list(root.glob("calibration_backup_*"))

            self.assertEqual(repaired.image_shape_hw, [120, 60])
            self.assertTrue(bool(result["calibration_resized"]))
            self.assertEqual(len(backups), 1)
            self.assertEqual(repaired.vials[0].roi_xywh[:2], [10, 10])
