...
from assay_tracking import (
    ...
    ensure_calibration_matches_shape,
...
class App(tk.Tk):
    ...
    def _build_vars(self) -> None:
        ...
        self.motor_pulse_ms_var = tk.StringVar(value="6000")
    ...
    def _build_profile_from_vars(self) -> AssayProfile:
        ...
        profile.motor.pulse_ms = self._int_var(self.motor_pulse_ms_var, 6000)
        profile.motor.pulse_user_configured = True
    ...
    def load_assay_calibration_into_editor(self, silent: bool = False) -> bool:
        try:
            ...
            calibration, resized, backup_path = ensure_calibration_matches_shape(path, expected_hw, save_backup=True)
            if resized:
                backup_msg = "" if backup_path is None else f" Backup saved to {backup_path}."
                self._log_assay(
                    f"Calibration shape was updated to match the current transformed background: {expected_hw}.{backup_msg}"
                )
