...
class SnapshotStitchTracker:
    """Per-snapshot stitch tracker with recent-lost reacquisition to suppress ghost IDs."""
    ...
    def __init__(
        ...
        *,
        ...
        reacquire_frames: int = 6,
        x_search_px: float = 26.0,
        y_search_px: float = 64.0,
        max_cost: float = 1.55,
        reacquire_x_search_px: float = 36.0,
        reacquire_y_search_px: float = 84.0,
        reacquire_max_cost: float = 1.85,
    ) -> None:
        self.calibration = calibration
        self.max_flies_per_vial = max(1, int(max_flies_per_vial))
        self.max_gap_frames = max(0, int(max_gap_frames))
        self.reacquire_frames = max(int(reacquire_frames), self.max_gap_frames + 1)
        self.x_search_px = float(x_search_px)
        self.y_search_px = float(y_search_px)
        self.max_cost = float(max_cost)
        self.reacquire_x_search_px = float(reacquire_x_search_px)
        self.reacquire_y_search_px = float(reacquire_y_search_px)
        self.reacquire_max_cost = float(reacquire_max_cost)
        self.active_tracks: Dict[int, List[Track]] = {int(v.physical_index): [] for v in calibration.enabled_vials}
        self.recently_lost_tracks: Dict[int, List[Track]] = {int(v.physical_index): [] for v in calibration.enabled_vials}
    ...
    def _cost_matrix(
        ...
        *,
        ...
        for ti, track in enumerate(tracks):
            ...
            missed_factor = min(float(max_missed_factor), float(max(0, track.missed_frames)))
            x_window = float(x_search_px) * (1.0 + 0.28 * missed_factor)
            y_window = float(y_search_px) * (1.0 + 0.38 * missed_factor)
            area_ref = None
            if track.history:
            ...
            time_penalty = min(0.30, float(time_penalty_scale) * missed_factor)
            for di, det in enumerate(detections):
                dx = abs(float(px) - float(det.center_xy_px[0])) / max(1.0, x_window)
                dy = abs(float(py) - float(det.center_xy_px[1])) / max(1.0, y_window)
                area_bias = 0.0 if area_ref is None else abs(float(det.area_px) - area_ref) / max(45.0, area_ref)
                cost[ti, di] = float(math.sqrt(dx * dx + dy * dy) + 0.08 * area_bias + time_penalty)
        return cost
    ...
    def _finalize(self, track: Track, *, frame_index: int, time_s: float) -> None:
        track.active = False
        track.retired_frame_index = int(frame_index)
        track.retired_time_s = float(time_s)
        self.completed_tracks.append(track)

    def _retire_to_recent(self, track: Track, *, frame_index: int, time_s: float) -> None:
        track.active = False
        track.retired_frame_index = int(frame_index)
        track.retired_time_s = float(time_s)
    ...
    def _purge_stale_recent_tracks(self, vial_index: int, frame_index: int, time_s: float) -> None:
        ...
        for track in recent_tracks:
            ...
            if int(frame_index - retired_at) > self.reacquire_frames:
                self._finalize(track, frame_index=frame_index, time_s=time_s)
            else:
    ...
    def update(self, *, frame_index: int, time_s: float, detections: Sequence[Detection], dt: float) -> List[Dict[str, Any]]:
        ...
        for vial in self.calibration.enabled_vials:
            ...
            self._purge_stale_recent_tracks(vial_index, frame_index=frame_index, time_s=time_s)
            vial_tracks = list(self.active_tracks.get(vial_index, []))
            recent_tracks = list(self.recently_lost_tracks.get(vial_index, []))
            vial_detections = list(detections_by_vial.get(vial_index, []))
            vial_detections.sort(key=lambda det: float(det.center_xy_px[1]), reverse=True)
            ...
            if vial_tracks and vial_detections:
                cost = self._cost_matrix(
                    ...
                    vial_detections,
                    time_s=float(time_s),
                    x_search_px=self.x_search_px,
                    y_search_px=self.y_search_px,
                    max_missed_factor=6.0,
                    time_penalty_scale=0.02,
                )
            ...
            for ti, track in enumerate(vial_tracks):
                if ti in matched_track_ids:
                    ...
                    if track.missed_frames > self.max_gap_frames:
                        self._retire_to_recent(track, frame_index=frame_index, time_s=time_s)
                    else:
                        survivors.append(track)

            available_det_ids = [idx for idx in range(len(vial_detections)) if idx not in matched_det_ids]
            recent_tracks = list(self.recently_lost_tracks.get(vial_index, []))
            if recent_tracks and available_det_ids:
                available_detections = [vial_detections[idx] for idx in available_det_ids]
                recent_cost = self._cost_matrix(
                    recent_tracks,
                    available_detections,
                    time_s=float(time_s),
                    x_search_px=self.reacquire_x_search_px,
                    y_search_px=self.reacquire_y_search_px,
                    max_missed_factor=10.0,
                    time_penalty_scale=0.04,
                )
                recent_matches = _greedy_match_pairs(recent_cost, self.reacquire_max_cost)
                if recent_matches:
                    revived_track_ids = {ti for ti, _ in recent_matches}
                    revived_detections = {di for _, di in recent_matches}
                    revived_tracks: List[Track] = []
                    for ti, di in recent_matches:
                        track = recent_tracks[ti]
                        det = available_detections[di]
                        self._revive_track(track, det, time_s=time_s)
                        revived_tracks.append(track)
                        rows.append(self._track_to_row(track))
                    survivors.extend(revived_tracks)
                    self.recently_lost_tracks[vial_index] = [
                        track for idx, track in enumerate(recent_tracks) if idx not in revived_track_ids
                    ]
                    available_det_ids = [available_det_ids[idx] for idx in range(len(available_det_ids)) if idx not in revived_detections]
                else:
                    self.recently_lost_tracks[vial_index] = recent_tracks

            recent_tracks = list(self.recently_lost_tracks.get(vial_index, []))
            unique_slots_left = max(0, int(self.max_flies_per_vial) - (int(self.next_display_id[vial_index]) - 1))
            held_slots = len(survivors) + len(recent_tracks)
    ...
    def finish(self) -> None:
        for vial_index in list(self.active_tracks.keys()):
            for track in self.active_tracks[vial_index]:
                last_frame = int(track.history[-1]["frame_index"]) if track.history else -1
                last_time = float(track.history[-1]["time_s"]) if track.history else 0.0
                self._finalize(track, frame_index=last_frame, time_s=last_time)
            self.active_tracks[vial_index] = []
        for vial_index in list(self.recently_lost_tracks.keys()):
            for track in self.recently_lost_tracks[vial_index]:
                last_frame = int(track.history[-1]["frame_index"]) if track.history else -1
                last_time = float(track.history[-1]["time_s"]) if track.history else 0.0
                self._finalize(track, frame_index=last_frame, time_s=last_time)
            self.recently_lost_tracks[vial_index] = []
...
def process_assay_run(
    ...
    *,
    ...
    allow_blob_split = bool(getattr(profile.detector, "allow_blob_split", True)) and not use_mask_blobs_as_flies
    blob_split_max_parts = int(getattr(profile.detector, "blob_split_max_parts", 4) or 4)
    ...
    snapshots_dir = ensure_dir(context.processing_dir / "snapshots") if save_snapshot_images else None
    next_snapshot_t = 0.0
    ...
    try:
        while True:
            ...
            if average_count <= 1:
                analysis_frame_bgr = apply_image_transform(raw_frame_bgr, context.transform)
            else:
            ...
            if save_snapshot_images and snapshots_dir is not None and source_time_s + 1e-9 >= next_snapshot_t:
    ...
    processing_meta: Dict[str, Any] = {
        ...
        "snapshot_dir": "" if snapshots_dir is None else str(snapshots_dir.resolve()),
    ...
    latest_processing = {
        ...
        "graphs_report_pdf": processing_meta.get("graphs_report_pdf", ""),
