...
class VialCalibration:
    ...
    threshold_point_px: Optional[List[int]] = None
    ...
    @property
    def threshold_y(self) -> int:
        if self.threshold_point_px and len(self.threshold_point_px) >= 2:
            return int(self.threshold_point_px[1])
        return int(round((self.top_y + self.baseline_y) / 2.0))

    @property
    def threshold_distance_px(self) -> float:
        return float(self.baseline_y - self.threshold_y)
...
class AssayCalibration:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AssayCalibration":
        ...
        for idx, item in enumerate(data.get("vials", []), start=1):
            ...
            payload.setdefault("threshold_point_px", None)
...
def resize_assay_calibration(calibration: AssayCalibration, new_shape_hw: Sequence[int]) -> AssayCalibration:
    old_h = max(1, int(calibration.image_shape_hw[0]))
    old_w = max(1, int(calibration.image_shape_hw[1]))
    new_h = max(1, int(new_shape_hw[0]))
    new_w = max(1, int(new_shape_hw[1]))
    ...
    def scale_point(point: Optional[Sequence[int]]) -> Optional[List[int]]:
        if not point or len(point) < 2:
            return None
        return [int(round(float(point[0]) * sx)), int(round(float(point[1]) * sy))]

    def scale_roi(roi_xywh: Sequence[int]) -> List[int]:
        x, y, w, h = [int(v) for v in roi_xywh[:4]]
        x2 = x + w
        y2 = y + h
        nx = int(round(float(x) * sx))
        ny = int(round(float(y) * sy))
        nx2 = int(round(float(x2) * sx))
        ny2 = int(round(float(y2) * sy))
        return [int(nx), int(ny), max(12, int(nx2 - nx)), max(12, int(ny2 - ny))]

    resized_vials: List[VialCalibration] = []
    for vial in calibration.vials:
        quad_points = getattr(vial, "quad_points_px", None)
        resized_vials.append(
            VialCalibration(
                physical_index=int(vial.physical_index),
                assay_index=vial.assay_index,
                enabled=bool(vial.enabled),
                roi_xywh=scale_roi(vial.roi_xywh),
                top_point_px=scale_point(vial.top_point_px) or list(vial.top_point_px),
                baseline_point_px=scale_point(vial.baseline_point_px) or list(vial.baseline_point_px),
                threshold_point_px=scale_point(vial.threshold_point_px) if vial.threshold_point_px else None,
                quad_points_px=None if not quad_points else [scale_point(point) or [int(point[0]), int(point[1])] for point in quad_points],
                tube_height_mm=vial.tube_height_mm,
                tube_width_mm=vial.tube_width_mm,
                label=vial.label,
    ...
    return normalize_assay_calibration(
        AssayCalibration(
            ...
            image_shape_hw=[int(new_h), int(new_w)],
            vials=resized_vials,
            ...
            ignored_physical_indices=list(calibration.ignored_physical_indices),
        )
    )


def ensure_calibration_matches_shape(
    calibration_path: str | Path,
    expected_shape_hw: Sequence[int],
    *,
    save_backup: bool = True,
) -> Tuple[AssayCalibration, bool, Optional[Path]]:
    path = Path(calibration_path).expanduser()
    calibration = load_assay_calibration(path)
    expected = [int(expected_shape_hw[0]), int(expected_shape_hw[1])]
    if list(calibration.image_shape_hw) == expected:
        return calibration, False, None

    resized = resize_assay_calibration(calibration, expected)
    backup_path: Optional[Path] = None
    if save_backup:
        backup_path = path.with_name(f"{path.stem}_backup_{calibration.image_shape_hw[0]}x{calibration.image_shape_hw[1]}_{timestamp_slug()}{path.suffix}")
        backup_path.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    save_assay_calibration(path, resized)
    return resized, True, backup_path
...
def _normalize_vial(vial: VialCalibration, image_shape_hw: Sequence[int], physical_index: int) -> VialCalibration:
    ...
    if quad_points is not None:
        ...
        default_threshold_y = int(round((top_y + base_y) / 2.0))
    else:
        ...
        default_threshold_y = int(round((top_y + base_y) / 2.0))
    threshold_point = getattr(vial, "threshold_point_px", None) or [int((top_x + base_x) / 2.0), default_threshold_y]
    threshold_x = _clamp(int(threshold_point[0]), x, x + w - 1)
    threshold_y = _clamp(int(threshold_point[1]), top_y + 1, base_y - 1) if base_y - top_y > 2 else top_y + 1
    return VialCalibration(
        ...
        threshold_point_px=[int(threshold_x), int(threshold_y)],
...
def calibrate_assay_interactive(
    ...
    for i, shape in enumerate(guided_shapes, start=1):
        ...
        threshold_point = [int((top_point[0] + base_point[0]) / 2.0), int(round((top_point[1] + base_point[1]) / 2.0))]
        vials.append(
            VialCalibration(
                physical_index=i,
                assay_index=None,
                ...
                threshold_point_px=threshold_point,
...
def _vial_polygon_mask(
    ...
    shrink_px = max(0, int(round(float(inner_margin_px) * 0.25)))
    if shrink_px > 0:
...
def _apply_vial_bounds_mask(
    ...
    clipped = mask.copy()
    edge_clip = max(0, int(round(float(inner_margin_px) * 0.5)))
    if edge_clip > 0:
        clipped[:, :edge_clip] = 0
        clipped[:, -edge_clip:] = 0
    return clipped
...
def _component_stats_from_binary(component_mask: np.ndarray) -> Optional[Tuple[int, int, int, int, float, float, int]]:
    ...
    cx = float(xs.mean())
    cy = float(ys.mean())
    return x0, y0, int(x1 - x0 + 1), int(y1 - y0 + 1), cx, cy, area


def _edge_distances_in_vial(vial: VialCalibration, x_px: float, y_px: float) -> Tuple[float, float, float, float]:
    quad_points = _vial_quad_points(vial)
    if quad_points is not None:
        tl, tr, br, bl = quad_points
        left_x = _line_x_at_y(tl, bl, y_px)
        ...
        width_px = max(1.0, float(right_x - left_x))
        height_px = max(1.0, float(bottom_y - top_y))
        side_edge_px = min(float(x_px) - float(left_x), float(right_x) - float(x_px))
        vertical_edge_px = min(float(y_px) - float(top_y), float(bottom_y) - float(y_px))
    else:
        roi_x, roi_y, roi_w, roi_h = vial.roi
        width_px = max(1.0, float(roi_w))
        height_px = max(1.0, float(roi_h))
        side_edge_px = min(float(x_px) - float(roi_x), float(roi_x + roi_w) - float(x_px))
        vertical_edge_px = min(float(y_px) - float(roi_y), float(roi_y + roi_h) - float(y_px))
    return max(0.0, float(side_edge_px)), max(0.0, float(vertical_edge_px)), float(width_px), float(height_px)


def _is_likely_vial_edge_artifact(
    vial: VialCalibration,
    x_px: float,
    y_px: float,
    bbox_w: int,
    ...
    side_edge_px, vertical_edge_px, _width_px, _height_px = _edge_distances_in_vial(vial, x_px, y_px)
    near_side = side_edge_px <= max(2.0, float(edge_guard_px))
    near_top_bottom = vertical_edge_px <= max(2.0, float(edge_guard_px) * 0.75)
    tall_ratio = float(bbox_h) / float(max(1, bbox_w))
    wide_ratio = float(bbox_w) / float(max(1, bbox_h))

    if near_side and tall_ratio >= 3.2 and bbox_h >= 12:
        return True
    if near_side and tall_ratio >= 2.4 and bbox_h >= 10 and area_px >= max(20, int(round(0.55 * bbox_w * bbox_h))):
        return True
    if near_top_bottom and wide_ratio >= 4.0 and bbox_w >= 14:
        return True
    return False


def _seed_mask_from_distance(component_mask: np.ndarray, expected_parts: int) -> np.ndarray:
    dist = cv2.distanceTransform(component_mask, cv2.DIST_L2, 5)
    if float(dist.max()) <= 0.0:
        return np.zeros_like(component_mask)
    threshold = max(0.8, float(dist.max()) * 0.45)
    seed_mask = np.uint8(dist >= threshold) * 255
    seed_mask = cv2.morphologyEx(seed_mask, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)))
    count, _labels = cv2.connectedComponents(seed_mask)
    if count - 1 >= 2:
        return seed_mask

    # Fallback: pick local maxima from the distance map with simple non-max suppression.
    work = dist.copy()
    seed_mask = np.zeros_like(component_mask)
    radius = max(2, int(round(min(component_mask.shape[:2]) * 0.08)))
    max_parts = max(2, int(expected_parts))
    for _ in range(max_parts):
        _min_val, max_val, _min_loc, max_loc = cv2.minMaxLoc(work)
        if float(max_val) <= 0.0:
            break
        cv2.circle(seed_mask, max_loc, radius, 255, -1)
        cv2.circle(work, max_loc, radius * 2, 0, -1)
    return seed_mask


def _split_large_component(component_mask: np.ndarray, expected_parts: int, max_parts: int = 4) -> List[Tuple[int, int, int, int, float, float, int]]:
    component_mask = np.uint8(component_mask > 0) * 255
    if cv2.countNonZero(component_mask) == 0:
        return []
    expected_parts = min(max(2, int(expected_parts)), max(2, int(max_parts)))
    seed_mask = _seed_mask_from_distance(component_mask, expected_parts=expected_parts)
    if cv2.countNonZero(seed_mask) == 0:
        return []

    seed_count, markers = cv2.connectedComponents(seed_mask)
    if seed_count - 1 < 2:
        return []
    markers = markers + 1
    unknown = cv2.subtract(component_mask, seed_mask)
    markers[unknown > 0] = 0
    gradient = cv2.cvtColor(cv2.GaussianBlur(component_mask, (5, 5), 0), cv2.COLOR_GRAY2BGR)
    markers = cv2.watershed(gradient, markers.astype(np.int32))

    parts: List[Tuple[int, int, int, int, float, float, int]] = []
    for label_id in range(2, int(markers.max()) + 1):
        part_mask = np.uint8(markers == label_id) * 255
        stats = _component_stats_from_binary(part_mask)
        if stats is not None:
            parts.append(stats)
    if len(parts) >= 2:
        return parts[:max_parts]

    # Watershed can fail on very smooth blobs. Fall back to K-means on pixel coordinates.
    ys, xs = np.where(component_mask > 0)
    if len(xs) < 4:
        return parts
    samples = np.column_stack([xs.astype(np.float32), ys.astype(np.float32)])
    k = min(expected_parts, max_parts, len(samples))
    if k < 2:
        return parts
    criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_MAX_ITER, 20, 0.5)
    _compactness, labels, _centers = cv2.kmeans(samples, k, None, criteria, 4, cv2.KMEANS_PP_CENTERS)
    km_parts: List[Tuple[int, int, int, int, float, float, int]] = []
    flat_labels = labels.reshape(-1)
    for cluster_id in range(k):
        cluster_mask = np.zeros_like(component_mask)
        cluster_pts = samples[flat_labels == cluster_id]
        if len(cluster_pts) == 0:
            continue
        cluster_xs = cluster_pts[:, 0].astype(int)
        cluster_ys = cluster_pts[:, 1].astype(int)
        cluster_mask[cluster_ys, cluster_xs] = 255
        cluster_mask = cv2.dilate(cluster_mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)), iterations=1)
        stats = _component_stats_from_binary(cluster_mask)
        if stats is not None:
            km_parts.append(stats)
    return km_parts[:max_parts] if len(km_parts) >= 2 else parts
...
def _component_detections_from_mask(
    ...
    *,
    ...
    count, labels_local, stats, centroids = cv2.connectedComponentsWithStats(mask, 8)
    out: List[Detection] = []
    ox, oy = offset_xy
    ...
    for i in range(1, int(count)):
        ...
        component_parts: List[Tuple[int, int, int, int, float, float, int]] = []
        if bool(allow_blob_split) and area > int(max_area):
            # Build a per-component mask for watershed / K-means splitting.
            component_mask = np.uint8(labels_local == i) * 255
            expected_parts = max(2, int(round(float(area) / max(1.0, float(max_area)))))
            component_parts = _split_large_component(
                component_mask,
                expected_parts=expected_parts,
                max_parts=max(2, int(blob_split_max_parts)),
            )
            if not component_parts:
                component_parts = [(x, y, w, h, float(centroids[i][0]), float(centroids[i][1]), int(area))]
        else:
            component_parts = [(x, y, w, h, float(centroids[i][0]), float(centroids[i][1]), int(area))]

        for part_x, part_y, part_w, part_h, cx, cy, part_area in component_parts:
            if int(part_area) < int(min_area):
                continue
            gx = float(ox + cx)
            gy = float(oy + cy)
            if _is_likely_vial_edge_artifact(
                vial,
                gx,
                gy,
                bbox_w=int(part_w),
                bbox_h=int(part_h),
                area_px=int(part_area),
                edge_guard_px=int(edge_guard_px),
            ):
                continue
            ...
            out.append(
                Detection(
                    ...
                    bbox_xywh=[int(ox + part_x), int(oy + part_y), int(part_w), int(part_h)],
                    center_xy_px=[gx, gy],
                    area_px=int(part_area),
                    frame_index=int(frame_index),
                    time_s=float(time_s),
...
def detect_assay_frame(
    ...
    for vial in calibration.enabled_vials:
        ...
        vial_dets = _component_detections_from_mask(
            ...
            allow_blob_split=allow_blob_split,
            blob_split_max_parts=blob_split_max_parts,
            edge_guard_px=max(2, int(round(float(inner_margin_px) * 1.25))) if int(inner_margin_px) > 0 else 2,
        )
        mask_nonzero = int(cv2.countNonZero(mask))
        need_relaxed_pass = (not vial_dets) or mask_nonzero < max(int(min_area) * 2, 18)
        if need_relaxed_pass:
            relaxed_threshold = float(max(6.0, min(threshold_used - 1.0, threshold_used * 0.72)))
            if relaxed_threshold + 0.25 < threshold_used:
                ...
                relaxed_mask = cv2.dilate(relaxed_mask, k2, iterations=1)
                ...
                if relaxed_dets and (not vial_dets or len(relaxed_dets) >= len(vial_dets)):
                    mask = relaxed_mask
...
def render_assay_calibration_overlay(
    ...
    for vial in calibration.vials:
        ...
        if show_threshold_lines:
            thr_color = (255, 90, 190)
            if _vial_quad_points(vial) is not None:
                pts = _vial_polygon_points(vial)
                xs = pts[:, 0]
                x0_thr = int(xs.min())
                x1_thr = int(xs.max())
                cv2.line(out, (x0_thr, int(vial.threshold_y)), (x1_thr, int(vial.threshold_y)), thr_color, 2, cv2.LINE_AA)
            else:
                cv2.line(out, (x, int(vial.threshold_y)), (x + w, int(vial.threshold_y)), thr_color, 2, cv2.LINE_AA)
            cv2.circle(out, (int(vial.center_x), int(vial.threshold_y)), 4, thr_color, -1)
        ...
        if show_threshold_lines:
            cv2.putText(out, "thr", (x + 6, max(y + 12, int(vial.threshold_y) - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.40, (255, 90, 190), 1, cv2.LINE_AA)
...
def annotate_assay_frame(
    ...
    calibration: AssayCalibration,
    detections: Sequence[Detection],
    tracker: Optional[MultiVialTracker],
    frame_index: int,
    time_s: float,
    show_positions: bool = False,
    frame_rows: Optional[Sequence[Dict[str, Any]]] = None,
    max_flies_per_vial: Optional[int] = None,
) -> np.ndarray:
    rows = list(frame_rows or [])
    if not rows and tracker is not None:
        rows = tracker.active_rows()
    detected_rows = [row for row in rows if bool(row.get("detected", True))]
    ...
    vial_capacity = int(max_flies_per_vial if max_flies_per_vial is not None else (tracker.max_flies_per_vial if tracker is not None else 0))
    ...
    for vial in calibration.enabled_vials:
        ...
        count_text = f"{count}" if vial_capacity <= 0 else f"{count}/{vial_capacity}"
    ...
    for row in detected_rows:
        ...
        label = f"{int(row['assay_tube_index'])}:{int(display_id)}" if display_id is not None else f"{int(row['assay_tube_index'])}:?"
        if show_positions:
        ...
        if all(key in row for key in ("bbox_x", "bbox_y", "bbox_w", "bbox_h")):
            bx = int(row.get("bbox_x", x - 4))
            by = int(row.get("bbox_y", y - 4))
            bw = max(1, int(row.get("bbox_w", 8)))
            bh = max(1, int(row.get("bbox_h", 8)))
        else:
            matched_det: Optional[Detection] = None
            for det in detections:
                if int(det.physical_vial_index) == int(row["physical_vial_index"]) and                    abs(float(det.center_xy_px[0]) - float(row["x_px"])) < 1.5 and                    abs(float(det.center_xy_px[1]) - float(row["y_px"])) < 1.5:
                    matched_det = det
                    break
        ...
        cv2.putText(out, label, (bx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.24, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(out, label, (bx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.24, (40, 40, 40), 1, cv2.LINE_AA)
    return out
...
def _render_pdf_text_page(pdf: PdfPages, title: str, lines: Sequence[str], *, fontsize: int = 11) -> None:
    ...
    for line in lines:
        line = str(line)
        wrapped.extend(textwrap.wrap(line, width=100, break_long_words=False, break_on_hyphens=False) or [""])
    page_text = "\n".join(wrapped)
    ax.text(0.03, 0.95, page_text, va="top", ha="left", fontsize=fontsize, family="monospace", linespacing=1.45)
...
def _render_pdf_table_pages(
    ...
    *,
    ...
    for col in display_df.columns:
        ...
        display_df[col] = display_df[col].map(lambda value: str(value))

    columns = list(display_df.columns)
    total_col_chunks = max(1, math.ceil(len(columns) / max_cols))
    total_row_chunks = max(1, math.ceil(len(display_df) / max_rows))

    for col_chunk_index, col_start in enumerate(range(0, len(columns), max_cols), start=1):
        subset_cols = columns[col_start: col_start + max_cols]
        col_df = display_df[subset_cols]
        for row_chunk_index, row_start in enumerate(range(0, len(col_df), max_rows), start=1):
            page_df = col_df.iloc[row_start: row_start + max_rows]
            fig = plt.figure(figsize=(11.0, 8.5))
            ax = fig.add_subplot(111)
            ax.axis("off")
            subtitle = (
                f"Columns {col_start + 1}-{col_start + len(subset_cols)} of {len(columns)} | "
                f"Rows {row_start + 1}-{row_start + len(page_df)} of {len(display_df)}"
            )
            ax.set_title(f"{title}\n{subtitle}", fontsize=14, pad=16)
            table = ax.table(
                cellText=page_df.values,
                colLabels=subset_cols,
                cellLoc="center",
                bbox=[0.02, 0.02, 0.96, 0.88],
            )
            table.auto_set_font_size(False)
            table.set_fontsize(8 if len(subset_cols) <= 5 else 7)
...
def _comparison_target_time_s(tracks_df: pd.DataFrame, session_meta: Dict[str, Any]) -> float:
    if not tracks_df.empty and "time_s" in tracks_df.columns:
        try:
            max_time = float(pd.to_numeric(tracks_df["time_s"], errors="coerce").max())
        except Exception:
            max_time = float("nan")
        if math.isfinite(max_time) and max_time > 0:
            return min(10.0, max_time)

    candidates: List[float] = []
    for key in ("processed_duration_s", "seconds", "assay_duration_s", "duration_s"):
    ...
    if not candidates:
        return 10.0
    return min(10.0, max(candidates))


def _build_x_displacement_summary_df(tracks_df: pd.DataFrame, *, target_time_s: float) -> pd.DataFrame:
    if tracks_df.empty or "x_from_left_px" not in tracks_df.columns:
        return pd.DataFrame()
    ...
    coverage_tolerance_s = 1.0 if float(target_time_s) >= 2.0 else max(0.2, float(target_time_s) / 4.0)
    per_track_rows: List[Dict[str, Any]] = []
    group_cols = [col for col in ("assay_tube_index", "internal_track_id") if col in working.columns]
    if len(group_cols) < 2:
        return pd.DataFrame()

    for (assay_tube_index, internal_track_id), group in working.groupby(group_cols, sort=True):
        g = group.sort_values("time_s").copy()
        start_time = float(pd.to_numeric(g["time_s"], errors="coerce").min())
        end_time = float(pd.to_numeric(g["time_s"], errors="coerce").max())
        if end_time <= start_time:
            continue
        if start_time > coverage_tolerance_s or end_time < float(target_time_s) - coverage_tolerance_s:
            continue

        start_idx = (pd.to_numeric(g["time_s"], errors="coerce") - 0.0).abs().idxmin()
        end_idx = (pd.to_numeric(g["time_s"], errors="coerce") - float(target_time_s)).abs().idxmin()
        start_row = g.loc[start_idx]
        end_row = g.loc[end_idx]
        try:
            dx_px = float(end_row["x_from_left_px"]) - float(start_row["x_from_left_px"])
        except Exception:
            continue
        row: Dict[str, Any] = {
            "assay_tube_index": int(assay_tube_index),
            ...
            "end_time_s": float(end_row["time_s"]),
            "dx_px": dx_px,
            "abs_dx_px": abs(dx_px),
        }
        if "x_from_left_mm" in g.columns and pd.notna(start_row.get("x_from_left_mm")) and pd.notna(end_row.get("x_from_left_mm")):
            dx_mm = float(end_row["x_from_left_mm"]) - float(start_row["x_from_left_mm"])
            row["dx_mm"] = dx_mm
            row["abs_dx_mm"] = abs(dx_mm)
    ...
    for assay_tube_index, group in per_track_df.groupby("assay_tube_index", sort=True):
        row: Dict[str, Any] = {
            "assay_tube_index": int(assay_tube_index),
            "flies_compared": int(group["internal_track_id"].nunique()),
            "mean_abs_dx_px": float(group["abs_dx_px"].mean()),
            "median_abs_dx_px": float(group["abs_dx_px"].median()),
            "mean_signed_dx_px": float(group["dx_px"].mean()),
            "comparison_target_s": float(target_time_s),
        }
        if "abs_dx_mm" in group.columns and group["abs_dx_mm"].notna().any():
            row["mean_abs_dx_mm"] = float(group["abs_dx_mm"].mean())
            row["median_abs_dx_mm"] = float(group["abs_dx_mm"].median())
            row["mean_signed_dx_mm"] = float(group["dx_mm"].mean())
        summary_rows.append(row)

    return pd.DataFrame(summary_rows).sort_values("assay_tube_index").reset_index(drop=True)


def _build_threshold_summary_df(per_vial_summary_df: pd.DataFrame) -> pd.DataFrame:
    if per_vial_summary_df.empty:
        return pd.DataFrame()
    desired_cols = [
        "assay_tube_index",
        "number_of_flies_detected",
        "number_of_unique_threshold_crossings",
        "fraction_crossing_by_10s",
        "first_crossing_time_s",
    ]
    keep_cols = [col for col in desired_cols if col in per_vial_summary_df.columns]
    if not keep_cols:
        return pd.DataFrame()
    return per_vial_summary_df[keep_cols].copy().sort_values("assay_tube_index").reset_index(drop=True)


def _build_threshold_events_df(threshold_crossings_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    if threshold_crossings_df is None or threshold_crossings_df.empty:
        return pd.DataFrame()
    working = threshold_crossings_df.copy()
    if "deduplicated" in working.columns:
        working = working[~working["deduplicated"].fillna(False).astype(bool)].copy()
    desired_cols = [
        "unique_event_id",
        "assay_tube_index",
        "display_id",
    ...
    keep_cols = [col for col in desired_cols if col in working.columns]
    if not keep_cols:
        return pd.DataFrame()
    return working[keep_cols].copy().sort_values(["assay_tube_index", "crossing_time_s", "unique_event_id"]).reset_index(drop=True)


def _build_velocity_summary_df(per_vial_summary_df: pd.DataFrame) -> pd.DataFrame:
    if per_vial_summary_df.empty:
        return pd.DataFrame()
    has_mm = (
        "mean_speed_mm_s" in per_vial_summary_df.columns
        and per_vial_summary_df["mean_speed_mm_s"].notna().any()
    )
    if has_mm:
        desired_cols = [
            "assay_tube_index",
            ...
            "mean_speed_mm_s",
            "max_speed_mm_s",
            "mean_vertical_velocity_mm_s",
            "max_vertical_velocity_mm_s",
        ]
    else:
        desired_cols = [
            "assay_tube_index",
            ...
            "mean_speed_px_s",
            "max_speed_px_s",
            "mean_vertical_velocity_px_s",
            "max_vertical_velocity_px_s",
        ]
    keep_cols = [col for col in desired_cols if col in per_vial_summary_df.columns]
    if not keep_cols:
        return pd.DataFrame()
...
def generate_graphs_and_pdf(
    ...
    *,
    ...
    if not vial_summary_df.empty and "assay_tube_index" in vial_summary_df.columns:
        assay_tube_count = int(vial_summary_df["assay_tube_index"].nunique())
    elif not tracks_df.empty and "assay_tube_index" in tracks_df.columns:
        assay_tube_count = int(tracks_df["assay_tube_index"].nunique())

    track_fragment_count = 0
    if not tracks_df.empty and "internal_track_id" in tracks_df.columns:
        track_fragment_count = int(tracks_df["internal_track_id"].nunique())
    elif not fly_summary_df.empty and "internal_track_id" in fly_summary_df.columns:
        track_fragment_count = int(fly_summary_df["internal_track_id"].nunique())

    threshold_events_df = _build_threshold_events_df(threshold_crossings_df)
    unique_crossing_total = int(threshold_events_df["unique_event_id"].nunique()) if not threshold_events_df.empty else 0
    target_time_s = _comparison_target_time_s(tracks_df, session_meta)
    displacement_df = _build_x_displacement_summary_df(tracks_df, target_time_s=float(target_time_s))
    threshold_summary_df = _build_threshold_summary_df(vial_summary_df)
    velocity_summary_df = _build_velocity_summary_df(vial_summary_df)

    with PdfPages(pdf_path) as pdf:
        cover_lines = [
            f"Session: {session_meta.get('session_name') or session_meta.get('run_dir', '')}",
            f"Run dir: {session_meta.get('run_dir', '')}",
            f"Raw video: {session_meta.get('raw_video_path', '')}",
            f"Background: {session_meta.get('background_path', '')}",
            f"Calibration: {session_meta.get('calibration_path', '')}",
            f"Duration (s): {session_meta.get('seconds') or session_meta.get('processed_duration_s') or session_meta.get('assay_duration_s') or session_meta.get('duration_s', '')}",
            f"Frames processed: {session_meta.get('frames_processed', '')}",
            f"Tracking mode: {session_meta.get('tracking_mode', '')}",
            f"Mask blobs as flies: {session_meta.get('use_mask_blobs_as_flies', '')}",
            f"Assay tubes: {assay_tube_count}",
            f"Track fragments detected: {track_fragment_count}",
            f"Unique threshold crossings: {unique_crossing_total}",
            f"0 s to {float(target_time_s):0.1f} s comparison window",
        ]
        ...
        if displacement_df.empty and threshold_summary_df.empty and threshold_events_df.empty and velocity_summary_df.empty:
            _render_pdf_text_page(
                pdf,
                "Fruit fly assay report",
                ["No assay summary tables were available for this run."],
                fontsize=12,
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_run.add_argument("--min-threshold", type=float, default=12.0)
