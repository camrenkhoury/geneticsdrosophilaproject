    ...
    with open_assay_camera(
        ...
        try:
            with VibrationMotor(profile.motor) as motor:
                if profile.motor.enabled:
                    logger(
                        f"Pulsing motor on GPIO {profile.motor.gpio_pin} for {profile.motor.pulse_ms} ms"
                    )
                motor.pulse()
        except MotorError:
            raise
        ...
        while True:
            ...
            if first_frame_shape is None:
                ...
                writer, raw_video_path = open_video_writer_with_path(raw_video_requested, fps=record_fps, frame_size=(w, h))
            ...
            if profile.outputs.save_preview_snapshots and t_now + 1e-9 >= next_snapshot_t:
