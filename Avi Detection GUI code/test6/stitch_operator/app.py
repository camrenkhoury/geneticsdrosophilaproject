...
class OperatorApp(tk.Tk):
    ...
    def _build_workflow_page(self, parent) -> tk.Frame:
        ...
        hero, body = self._card(
            page,
            "Unified flow",
            "Start the automated cycle here, keep the channel and chamber in view, and only move into the other tabs when you need setup, assay work, or recovery.",
        )
        ...
        previews.rowconfigure(0, weight=5)
        previews.rowconfigure(1, weight=4)
        self._image_panel(previews, "workflow_channel", "Channel view", width=1140, height=360).grid(
            row=0, column=0, columnspan=2, sticky="nsew", pady=(0, 14)
        )
        self._image_panel(previews, "workflow_sexing", "Sexing chamber", width=700, height=360).grid(
            row=1, column=0, sticky="nsew", padx=(0, 10)
        )
        self._image_panel(previews, "workflow_assay", "Assay preview", width=700, height=360).grid(
            row=1, column=1, sticky="nsew", padx=(10, 0)
        )

        lower = tk.Frame(page, bg=PALETTE.background)
        lower.grid(row=2, column=0, sticky="ew", pady=(18, 0))
        ...
        vial_section, vial_body = self._card(
            lower,
            "Assay destinations",
            "Counts remain visible here while denser diagnostics and manual controls stay contained in Debug / Advanced.",
        )
    ...
    def _build_channel_page(self, parent) -> tk.Frame:
        ...
        self._image_panel(page, "channel_main", "Channel capture", width=1100, height=400).grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        right = tk.Frame(page, bg=PALETTE.background)
        right.grid(row=0, column=1, sticky="nsew")

        setup_card, setup_body = self._card(right, "Channel setup", "Capture a background once, calibrate once, then use Capture Channel during operation. The preview on this page keeps the full horizontal channel scene in view.")
    ...
    def _build_loading_page(self, parent) -> tk.Frame:
        ...
        self._image_panel(left, "loading_sexing", "Sexing chamber", width=960, height=640).pack(fill="both", expand=True)
    ...
    def _build_debug_page(self, parent) -> tk.Frame:
        ...
        tuning_card, tuning_body = self._card(body, "Loading tuning", "Adjust pickup offset, hold times, release settle time, and key positions here. Save applies immediately and persists to operator_settings.json.")
        ...
        model_card.grid(row=4, column=0, sticky="ew", pady=(0, 18))
        ...
        settings_card.grid(row=5, column=0, sticky="nsew", pady=(0, 18))
        ...
        state_card.grid(row=6, column=0, sticky="nsew", pady=(0, 18))
    ...
    def _save_tuning_fields(self):
        ...
        try:
            ...
            self._reload_settings_editor_from_controller()
            self._reload_tuning_vars_from_controller()
            self.footer_var.set("Loading tuning saved.")
        except Exception as exc:
    ...
    def _refresh_images(self, snapshot: OperatorState):
        ...
        self._update_image_widget("workflow_assay", snapshot.assay.preview_image_path, "Run an assay or capture an assay preview to populate this stage.")
