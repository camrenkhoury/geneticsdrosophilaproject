...
def generate_graphs_and_pdf(
    ...
    with PdfPages(pdf_path) as pdf:
        cover_lines = [
            ...
            f"Assay tubes tracked: {session_meta.get('tubes_tracked') or session_meta.get('tracks_detected_total', '')}",
        ]
        _render_pdf_text_page(pdf, "Fruit fly assay report", cover_lines, fontsize=11)

        if not vial_summary_df.empty:
            _render_pdf_table_pages(pdf, "Per-vial summary", vial_summary_df, max_rows=14, max_cols=6)
        ...
        for assay_tube_index, grp in tracks_df.groupby("assay_tube_index", sort=True):
            ...
            if handles:
                ax.legend(handles, labels, loc="upper left", bbox_to_anchor=(1.01, 1.0), fontsize=8, borderaxespad=0.0)
        ...
        if not fly_summary_df.empty:
            _render_pdf_table_pages(pdf, "Per-fly summary", fly_summary_df, max_rows=14, max_cols=6)

            value_col = None
            for candidate in ("max_height_mm", "max_height_px", "max_distance_mm", "max_distance_px"):
                if candidate in fly_summary_df.columns:
                    value_col = candidate
                    break
            if value_col is not None:
                ...
                labels = fly_summary_df["label"].astype(str).tolist()
