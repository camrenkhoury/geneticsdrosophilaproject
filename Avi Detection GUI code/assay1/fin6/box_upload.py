...
def _replace_if_placeholder(base_value: str, replacement: str) -> str:
    return replacement if base_value.startswith("PASTE_") and replacement else base_value
...
def _pick_text(
    ...
    *,
    ...
    if key == "tokens_file" and base_value and legacy_cfg and legacy_cfg.get("tokens_file"):
        try:
            if not Path(base_value).expanduser().exists():
                base_value = ""
        except Exception:
            pass
    if base_value not in (None, "") and base_value != baseline:
        return str(base_value)
    file_value = str(file_cfg.get(key, "") or "")
    if key == "tokens_file" and file_value and legacy_cfg and legacy_cfg.get("tokens_file"):
        try:
    ...
    if legacy_cfg:
        legacy_value = legacy_cfg.get(key, "")
