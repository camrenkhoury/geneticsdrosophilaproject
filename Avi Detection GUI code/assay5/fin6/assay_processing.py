...
def _integrate_metric_over_time(per_time: pd.DataFrame, value_col: str) -> float:
    if per_time.empty or "time_s" not in per_time.columns or value_col not in per_time.columns:
        return 0.0
    times = pd.to_numeric(per_time["time_s"], errors="coerce").to_numpy(dtype=float)
    values = pd.to_numeric(per_time[value_col], errors="coerce").to_numpy(dtype=float)
    mask = np.isfinite(times) & np.isfinite(values)
    if int(mask.sum()) < 2:
        return 0.0
    if hasattr(np, "trapezoid"):
        return float(np.trapezoid(values[mask], times[mask]))
    return float(sum((times[idx] - times[idx - 1]) * (values[idx] + values[idx - 1]) * 0.5 for idx in range(1, len(times[mask]))))
...
def _build_vial_summaries(
    ...
    configured_max_flies_per_vial: int,
    ...
    for assay_tube_index, group in track_frames_df.groupby("assay_tube_index", sort=True):
        ...
        configured_flies = max(1, int(configured_max_flies_per_vial))
        per_time = (
        ...
        raw_flies_detected = int(summary_grp["internal_track_id"].nunique())
        flies_detected = min(raw_flies_detected, configured_flies)
        max_simultaneous_tracked = int(g.groupby("time_s")["internal_track_id"].nunique().max()) if not g.empty else 0
        raw_unique_crossing_count = int(unique_crossings["unique_event_id"].nunique())
        unique_crossing_count = min(raw_unique_crossing_count, configured_flies)
        first_crossing_time_s = None
        if not unique_crossings.empty and "crossing_time_s" in unique_crossings.columns:
            first_crossing_time_s = float(unique_crossings["crossing_time_s"].min())
        summary_row: Dict[str, Any] = {
            "assay_tube_index": int(assay_tube_index),
            "configured_flies_per_vial": configured_flies,
            "number_of_flies_detected": flies_detected,
            "flies_detected": flies_detected,
            "track_fragments_detected": raw_flies_detected,
            "max_simultaneous_tracked_flies": max_simultaneous_tracked,
            "raw_unique_threshold_crossings": raw_unique_crossing_count,
            "number_of_unique_threshold_crossings": unique_crossing_count,
            "unique_threshold_crossings": unique_crossing_count,
            "fraction_crossing_by_10s": float(unique_crossing_count) / float(configured_flies),
            "first_crossing_time_s": first_crossing_time_s,
            "estimated_average_distance_travelled_px": _integrate_metric_over_time(per_time, "mean_speed_px_s"),
            "estimated_mean_velocity_px_s": float(per_time["mean_speed_px_s"].mean(skipna=True)),
            "peak_estimated_velocity_px_s": float(per_time["max_speed_px_s"].max(skipna=True)),
            "mean_height_px": float(g["distance_from_base_px"].mean(skipna=True)),
            "max_height_px": float(g["distance_from_base_px"].max(skipna=True)),
        ...
        if g["distance_from_base_mm"].notna().any():
            summary_row.update(
                {
                    "estimated_average_distance_travelled_mm": _integrate_metric_over_time(per_time, "mean_speed_mm_s"),
                    "estimated_mean_velocity_mm_s": float(per_time["mean_speed_mm_s"].mean(skipna=True)),
                    "peak_estimated_velocity_mm_s": float(per_time["max_speed_mm_s"].max(skipna=True)),
                    "mean_height_mm": float(g["distance_from_base_mm"].mean(skipna=True)),
                    "max_height_mm": float(g["distance_from_base_mm"].max(skipna=True)),
...
def process_assay_run(
    ...
    *,
    ...
    save_annotated_video = bool(getattr(profile.analysis, "save_annotated_video", False))
    save_mask_video = bool(profile.analysis.save_mask_video)
    save_snapshots = bool(profile.outputs.save_preview_snapshots)
    needs_annotated_preview = bool(save_annotated_video or save_snapshots or progress_callback is not None)
    needs_mask_preview = bool(save_mask_video or save_snapshots or progress_callback is not None)
    ...
    try:
        while True:
            ...
            mask_bgr = assay_mask_to_bgr(mask, frame_bgr=aligned_bgr) if needs_mask_preview else None
            annotated_bgr = None
            if needs_annotated_preview:
            ...
            if save_annotated_video:
                ...
                assert annotated_writer is not None and annotated_bgr is not None
                annotated_writer.write(annotated_bgr)
            if save_mask_video:
                if mask_writer is None:
                    assert mask_bgr is not None
                ...
                assert mask_writer is not None and mask_bgr is not None
                mask_writer.write(mask_bgr)

            if save_snapshots and source_time_s + 1e-9 >= next_snapshot_t:
                if snapshots_dir is None:
                    snapshots_dir = ensure_dir(context.processing_dir / "snapshots")
                assert annotated_bgr is not None and mask_bgr is not None
            ...
            if progress_callback is not None:
                preview_bgr = annotated_bgr if annotated_bgr is not None else aligned_bgr
                preview_mask_bgr = mask_bgr if mask_bgr is not None else assay_mask_to_bgr(mask, frame_bgr=aligned_bgr)
                progress_callback(
                    {
                        "frame_index": int(processed_frame_index),
                        "source_frame_index": int(source_frame_index),
    ...
    per_vial_summary_df, vial_velocity_timeseries_df = _build_vial_summaries(
        ...
        configured_max_flies_per_vial=int(profile.detector.max_flies_per_vial),
    )
    ...
    processing_meta: Dict[str, Any] = {
        ...
        "save_mask_video": bool(save_mask_video),
        "max_flies_per_vial": int(profile.detector.max_flies_per_vial),
