...
class BoxUploadTests(unittest.TestCase):
    def test_write_box_templates_and_resolve_effective_settings(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(effective.upload_after_processing)
            self.assertFalse(effective.upload_after_recording)
            self.assertEqual(effective.artifact_mode, "summaries")
            self.assertEqual(Path(effective.tokens_file), tokens_path)
            self.assertFalse(should_auto_upload({"config_file": str(config_path)}, "processing"))
            self.assertFalse(should_auto_upload({"config_file": str(config_path)}, "recording"))

    def test_explicit_profile_bools_override_true_config_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            effective = resolve_effective_box_settings(
                {
    ...
    def test_collect_artifacts_filters_backgrounds(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (run_dir / "processed" / "snapshots").mkdir(parents=True)
            for file_path in [
                ...
                run_dir / "processed" / "snapshots" / "snapshot_000_raw.png",
                run_dir / "processed" / "snapshot_index.csv",
            ...
            self.assertNotIn("snapshot_000_raw.png", summary_names)
            self.assertNotIn("snapshot_index.csv", summary_names)
            ...
            self.assertNotIn("snapshot_000_raw.png", video_names)
            self.assertNotIn("snapshot_index.csv", video_names)
    ...
    def test_collect_artifacts_core_mode_uses_latest_processing(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            save_json(
                run_dir / "run_manifest.json",
                {"raw_video_path": str((run_dir / "raw_video.mp4").resolve())},
            )
            save_json(run_dir / "processed" / "latest_processing.json", {"processing_dir": str(proc_b.resolve())})
