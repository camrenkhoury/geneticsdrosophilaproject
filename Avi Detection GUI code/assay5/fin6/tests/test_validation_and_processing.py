...
class ValidationAndProcessingTests(unittest.TestCase):
    ...
    def test_no_crossing_run_still_processes_without_tube_index_errors(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            profile.analysis.save_annotated_video = True
    ...
    def test_processing_can_skip_annotated_video_and_snapshots(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            run_dir = root / "assay_20260409_120002"
            run_dir.mkdir(parents=True, exist_ok=True)

            profile = AssayProfile(name="synthetic_summary_only")
            profile.assay_camera.fps = 10.0
            profile.assay_duration_s = 2.0
            profile.analysis.analysis_fps = 5.0
            profile.analysis.alignment_enabled = False
            profile.analysis.save_annotated_video = False
            profile.analysis.save_mask_video = False
            profile.outputs.save_preview_snapshots = False
            profile.detector.min_area = 8
            profile.detector.max_area = 500
            profile.detector.min_threshold = 10.0
            profile.detector.inner_margin_px = 0
            ...
            self.assertFalse(bool(result.get("annotated_video_path")))
            self.assertFalse((Path(result["processing_json"]).parent / "annotated_video.mp4").exists())
            self.assertFalse((Path(result["processing_json"]).parent / "mask_video.mp4").exists())
            self.assertFalse((Path(result["processing_json"]).parent / "snapshots").exists())
            self.assertTrue(Path(result["report_pdf"]).exists())
