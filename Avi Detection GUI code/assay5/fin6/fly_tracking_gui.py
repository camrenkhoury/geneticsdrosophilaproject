...
class App(tk.Tk):
    ...
    def _build_vars(self) -> None:
        ...
        self.assay_save_annotated_video_var = tk.BooleanVar(value=False)
        ...
        self.assay_save_snapshots_var = tk.BooleanVar(value=False)
    ...
    def _build_processing_card(self, parent, row: int) -> int:
        ...
        ttk.Checkbutton(card, text="Save annotated video", variable=self.assay_save_annotated_video_var).grid(row=5, column=1, sticky="w", padx=10, pady=3)
        ttk.Checkbutton(card, text="Save mask video", variable=self.assay_save_mask_video_var).grid(row=5, column=2, sticky="w", padx=10, pady=3)
        ttk.Checkbutton(card, text="Show positions", variable=self.assay_show_positions_var).grid(row=5, column=3, sticky="w", padx=10, pady=3)
    ...
    def _default_profile(self, name: str = "default") -> AssayProfile:
        ...
        profile.box_upload.enabled = False
        profile.box_upload.upload_after_processing = False
        return profile
    ...
    def load_profile(self, profile: AssayProfile) -> None:
        ...
        self.assay_save_annotated_video_var.set(bool(getattr(profile.analysis, "save_annotated_video", False)))
    ...
    def _build_profile_from_vars(self) -> AssayProfile:
        ...
        profile.analysis.save_annotated_video = bool(self.assay_save_annotated_video_var.get())
    ...
    def _on_assay_process_done(self, result: Dict[str, Any]) -> None:
        ...
        elif result.get("mask_video_path"):
            self.assay_preview_mode_var.set("mask")
            self.assay_preview_info_var.set(f"Processed run: {Path(run_dir).name}")
            self.playback_status_var.set("Mask video ready. Press Play mask to review it inside the GUI.")
        else:
            self.assay_preview_mode_var.set("raw" if result.get("raw_video_path") else "calibration")
            self.assay_preview_info_var.set(f"Processed run: {Path(run_dir).name}")
            self.playback_status_var.set("Summary outputs are ready. Use Play raw to review the recording, or enable Save annotated video for processed playback.")
