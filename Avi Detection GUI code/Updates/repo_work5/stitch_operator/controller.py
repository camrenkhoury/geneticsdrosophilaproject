...
class WorkflowController:
    ...
    def _auto_route_from_source_impl(
        ...
        *,
        ...
        after_result = self.channel.capture_channel()
        ...
        success = (after_count < before_count) or (not source_still_present)
    ...
    def start_auto_flow(self) -> bool:
        def task():
            ...
            while True:
                ...
                if attempt.get("outcome") == "routed":
                    ...
                    if (
                        ...
                        self._update(status_message=skip_message, current_target="")
            ...
            self._set_stage(
                ...
                next_action="Capture Channel",
                message=(
                    f"Channel clear. Auto loading complete. Routed {routed_count} flies. "
                    "Assay recording is temporarily disabled in this build."
                ),
            )
            self.log(f"Auto flow finished. Routed {routed_count} flies.")
    ...
    def start_calibrate_assay(self) -> bool:
        def task():
            ...
            self._set_stage(
                WorkflowStage.ASSAY,
                "Assay calibrated",
                next_action="Await New Assay Code",
    ...
    def start_run_assay(self) -> bool:
        def task():
            raise OperatorFacingError(
                "Assay recording is temporarily disabled while the new recording code is being integrated."
            )

        return self._begin_task(
            "run_assay",
            WorkflowStage.ASSAY,
            "Assay recording disabled",
            "Assay recording is temporarily disabled in this build.",
            task,
        )
