...
from brio_channel_cli import calibrate_channel, capture_brio_background
from camera_sources import BrioCamera, BrioConfig, describe_camera_selection, list_video_devices
...
class App(tk.Tk):
    ...
    def _build_vars(self) -> None:
        ...
        self.detector_min_threshold_var = tk.StringVar(value="16")
    ...
    def _build_ui(self) -> None:
        ...
        self.notebook.add(assay_tab, text="Assay Workflow")
    ...
    def _build_assay_tab(self, parent) -> None:
        ...
        canvas = tk.Canvas(sidebar, background=CARD_BG, highlightthickness=0, borderwidth=0)
        scrollbar = ttk.Scrollbar(sidebar, orient="vertical", command=canvas.yview)
        scroll_body = ttk.Frame(canvas, style="Card.TFrame")
        scroll_body.bind("<Configure>", lambda _e: canvas.configure(scrollregion=canvas.bbox("all")))
        ...
        row = 0
        row = self._build_profile_card(scroll_body, row)
        row = self._build_background_card(scroll_body, row)
        row = self._build_transform_card(scroll_body, row)
        row = self._build_calibration_card(scroll_body, row)
        row = self._build_recording_card(scroll_body, row)
        row = self._build_processing_card(scroll_body, row)
        row = self._build_upload_card(scroll_body, row)
        ...
        ttk.Label(viewer, textvariable=self.assay_preview_info_var, style="Muted.TLabel").pack(anchor="w", padx=10, pady=(0, 6))

        lower = ttk.Panedwindow(viewer, orient="horizontal")
        lower.pack(fill="both", expand=False, padx=10, pady=(0, 10))
        ...
        ttk.Label(region_card, text="Calibrated vials", style="Title.TLabel").pack(anchor="w", padx=6, pady=(4, 2))
        self.region_tree = ttk.Treeview(region_card, columns=("idx", "label", "state", "thr"), show="headings", height=8)
        for col, text, width in [("idx", "#", 40), ("label", "Label", 120), ("state", "State", 70), ("thr", "Thr y", 60)]:
        ...
        form = ttk.Frame(region_card, style="Card.TFrame")
        ...
        apply_row = ttk.Frame(region_card, style="Card.TFrame")
        apply_row.pack(fill="x", padx=6, pady=(0, 6))
        ttk.Button(apply_row, text="Apply selected region", command=self.apply_selected_region_form).pack(side="left", padx=(0, 4))
        ttk.Button(apply_row, text="Delete", command=self.assay_canvas.delete_selected).pack(side="left", padx=4)
        ttk.Button(apply_row, text="Duplicate", command=self.assay_canvas.duplicate_selected).pack(side="left", padx=4)
        ttk.Button(apply_row, text="Enable/Ignore", command=self.assay_canvas.toggle_selected_enabled).pack(side="left", padx=4)
    ...
    def _build_profile_card(self, parent, row: int) -> int:
        ...
        self._grid_labeled_entry(card, 3, 0, "Assay device", self.assay_device_var)
        self._grid_labeled_entry(card, 3, 2, "Assay hint", self.assay_preferred_hint_var)
        self._grid_labeled_entry(card, 4, 0, "Channel device", tk.StringVar(value="auto:channel"), state="disabled")
        self._grid_labeled_entry(card, 4, 2, "Profile path", self.profile_path_var, state="readonly")
        ttk.Label(card, textvariable=self.channel_camera_status_var, style="Muted.TLabel").grid(row=5, column=0, columnspan=4, sticky="w", padx=10, pady=(2, 0))
        ttk.Label(card, textvariable=self.assay_camera_status_var, style="Muted.TLabel").grid(row=6, column=0, columnspan=4, sticky="w", padx=10, pady=(0, 4))
    ...
    def _build_calibration_card(self, parent, row: int) -> int:
        ...
        ttk.Button(action_row, text="Clear", command=self.assay_canvas.clear_regions).pack(side="left", padx=4)
        ttk.Button(action_row, text="Sort left-right", command=self.assay_canvas.sort_left_to_right).pack(side="left", padx=4)
        ttk.Button(action_row, text="Copy refs to all", command=self.assay_canvas.apply_reference_style_from_selected).pack(side="left", padx=4)
        return row + 1
    ...
    def _build_recording_card(self, parent, row: int) -> int:
        ...
        actions = ttk.Frame(card, style="Card.TFrame")
        actions.grid(row=5, column=0, columnspan=4, sticky="ew", padx=10, pady=(4, 10))
    ...
    def _build_processing_card(self, parent, row: int) -> int:
        card = self._card(parent, row, "Processing", "Offline processing uses the saved run snapshots and selected analysis settings.")
        ...
        ttk.Checkbutton(card, text="Alignment enabled", variable=self.assay_alignment_var).grid(row=4, column=0, sticky="w", padx=10, pady=3)
        ttk.Checkbutton(card, text="Save mask video", variable=self.assay_save_mask_video_var).grid(row=4, column=1, sticky="w", padx=10, pady=3)
        ttk.Checkbutton(card, text="Show positions", variable=self.assay_show_positions_var).grid(row=4, column=2, sticky="w", padx=10, pady=3)
        ttk.Label(card, textvariable=self.profile_last_run_var, style="Muted.TLabel").grid(row=5, column=0, columnspan=4, sticky="w", padx=10, pady=(2, 4))
        actions = ttk.Frame(card, style="Card.TFrame")
    ...
    def _build_upload_card(self, parent, row: int) -> int:
        card = self._card(parent, row, "Export / Upload", "Optional Box upload for summaries, videos, or the full session folder.")
    ...
    def _card(self, parent, row: int, title: str, subtitle: str) -> ttk.Frame:
        ...
        ttk.Label(card, text=title, style="Title.TLabel").grid(row=0, column=0, columnspan=4, sticky="w", padx=10, pady=(8, 0))
        ttk.Label(card, text=subtitle, style="Muted.TLabel").grid(row=0, column=0, columnspan=4, sticky="w", padx=140, pady=(8, 0))
        for col in range(4):
    ...
    def load_profile(self, profile: AssayProfile) -> None:
        ...
        self.assay_device_var.set(str(profile.assay_camera.device))
        ...
        self.box_tokens_file_var.set(profile.box_upload.tokens_file)
        self.box_config_file_var.set(profile.box_upload.config_file)
    ...
    def _build_profile_from_vars(self) -> AssayProfile:
        ...
        profile.assay_camera.device = self.assay_device_var.get().strip() or "auto:assay"
        ...
        profile.detector.min_threshold = self._float_var(self.detector_min_threshold_var, 16.0)
        ...
        profile.box_upload.tokens_file = self.box_tokens_file_var.get().strip()
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self.app_status_var.set("Motor test complete")
                    self._log_assay("Motor test complete")
    ...
    def _refresh_device_labels(self) -> None:
        ...
        channel_desc = describe_camera_selection("auto:channel", role="channel", preferred_hint=profile.channel_camera.preferred_hint)
        ...
        if channel_desc is None:
            self.channel_camera_status_var.set("Channel camera: not resolved")
        else:
            self.channel_camera_status_var.set(f"Channel camera: {channel_desc.card_name} -> {channel_desc.stable_path}")
        if assay_desc is None:
            self.assay_camera_status_var.set("Assay camera: not resolved")
        else:
    ...
    def _update_region_tree(self, select_canvas: bool = False) -> None:
        ...
        if select_canvas and self.assay_canvas.selected_index is not None and tree.exists(str(self.assay_canvas.selected_index)):
            tree.selection_set(str(self.assay_canvas.selected_index))
    ...
    def _on_assay_record_done(self, result: Dict[str, Any]) -> None:
        ...
        if self.box_enabled_var.get() and self.box_upload_after_recording_var.get():
    ...
    def upload_last_run(self, auto: bool = False) -> None:
        ...
        self._start_worker(
            ...
            run_dir,
            profile.box_upload,
            artifact_mode=profile.box_upload.artifact_mode,
            logger=self._thread_logger("assay"),
