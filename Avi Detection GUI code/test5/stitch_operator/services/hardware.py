...
class HardwareService:
    ...
    # ------------------------------------------------------------------
    def move_to_pickup(self, pickup_position_mm: float, *, vacuum_pick_delay_s: float) -> float:
        self.move_absolute(pickup_position_mm)
        self.vacuum_on()
        time.sleep(max(0.0, float(vacuum_pick_delay_s)))
        return self.position_mm

    def drop_in_chamber_and_clear(self, *, vacuum_drop_delay_s: float) -> float:
        self.move_to_chamber()
        self.vacuum_off()
        time.sleep(max(0.0, float(vacuum_drop_delay_s)))
        return self.move_absolute(self.chamber_clear_position())

    def reacquire_from_chamber(self, *, vacuum_pick_delay_s: float) -> float:
        self.move_to_chamber()
        self.vacuum_on()
        time.sleep(max(0.0, float(vacuum_pick_delay_s)))
        return self.position_mm

    def drop_into_vial(self, destination_mm: float, *, vacuum_drop_delay_s: float) -> float:
