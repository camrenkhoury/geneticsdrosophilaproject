...
class OperatorApp(tk.Tk):
    ...
    def _image_panel(self, parent, key: str, title: str, *, width: int = 520, height: int = 300) -> tk.Frame:
        ...
        panel = tk.Label(body, text="No preview available", bg="#000000", fg="#d0d0d0", font=BODY_FONT, anchor="center", justify="center")
        panel.pack(fill="both", expand=True)
        self._image_widgets[key] = {"label": panel, "photo": None, "size": (width, height), "last_path": "", "last_mtime": None, "last_box": None}
        return outer
    ...
    def _build_workflow_page(self, parent) -> tk.Frame:
        ...
        hero, body = self._card(
            page,
            "Unified flow",
            "The workflow screen is the main operator surface: start the automated cycle, monitor the live channel and chamber views, and only drop into other tabs when needed.",
        )
        ...
        guidance = tk.Frame(body, bg=PALETTE.surface_lowest, padx=18, pady=16)
        ...
        tk.Label(guidance, textvariable=self.workflow_message_value, bg=PALETTE.surface_lowest, fg=PALETTE.text, font=("Inter", 18, "bold"), justify="left", wraplength=760).pack(anchor="w", pady=(8, 6))
        guidance_meta = tk.Frame(guidance, bg=PALETTE.surface_lowest)
        guidance_meta.pack(fill="x", pady=(8, 0))
        stage_box = tk.Frame(guidance_meta, bg=PALETTE.surface, padx=14, pady=12)
        stage_box.pack(side="left", fill="y")
        tk.Label(stage_box, text="Current stage", bg=PALETTE.surface, fg=PALETTE.text_muted, font=LABEL_FONT).pack(anchor="w")
        tk.Label(stage_box, textvariable=self.workflow_stage_value, bg=PALETTE.surface, fg=PALETTE.text, font=SECTION_FONT, wraplength=220, justify="left").pack(anchor="w", pady=(6, 0))
        next_box = tk.Frame(guidance_meta, bg=PALETTE.surface, padx=14, pady=12)
        next_box.pack(side="left", fill="y", padx=(12, 0))
        tk.Label(next_box, text="Next action", bg=PALETTE.surface, fg=PALETTE.text_muted, font=LABEL_FONT).pack(anchor="w")
        tk.Label(next_box, textvariable=self.workflow_next_value, bg=PALETTE.surface, fg=PALETTE.text, font=SECTION_FONT, wraplength=240, justify="left").pack(anchor="w", pady=(6, 0))
        ...
        for idx, (label, cmd, primary) in enumerate([
            ...
            self._action_button(secondary_actions, label, cmd, primary=primary, width=14).grid(row=idx // 2, column=idx % 2, padx=6, pady=6, sticky="ew")
        ...
        left.columnconfigure(0, weight=5)
        left.columnconfigure(1, weight=4)
        left.rowconfigure(0, weight=1)
        self._image_panel(left, "workflow_channel", "Channel view", width=860, height=430).grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        self._image_panel(left, "workflow_sexing", "Sexing view", width=620, height=430).grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        ...
        vial_section, vial_body = self._card(page, "Assay destinations", "Counts stay visible on the main screen while deeper diagnostics remain in Debug / Advanced.")
        vial_section.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(18, 0))
    ...
    def _build_debug_page(self, parent) -> tk.Frame:
        ...
        note_card, note_body = self._card(body, "Debug / Advanced", "This is the containment zone for dense controls, manual recovery, camera paths, settings, logs, and raw state.")
        note_card.grid(row=0, column=0, sticky="ew", pady=(0, 18))
        tk.Label(note_body, text="Use these controls for troubleshooting, validation, calibration recovery, direct hardware actions, and settings persistence.", bg=PALETTE.surface_low, fg=PALETTE.text_muted, font=BODY_FONT).pack(anchor="w")
        ...
        settings_card.grid(row=4, column=0, sticky="nsew", pady=(0, 18))
    ...
    def _refresh_images(self, snapshot: OperatorState):
        ...
        self._update_image_widget("workflow_channel", channel_scene, "Capture the channel to preview the full channel scene.")
        self._update_image_widget("workflow_sexing", snapshot.sexing.image_path, "Route a fly to view the sexing chamber.")
        self._update_image_widget("channel_main", channel_scene, "No channel preview available.")
