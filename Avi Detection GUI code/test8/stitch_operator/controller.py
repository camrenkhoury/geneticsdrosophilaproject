...
class WorkflowController:
    ...
    def _route_next_fly_impl(
        ...
        *,
        ...
        chamber_count = int(sex_result.get("count", 0) or 0)
        self.log(
            f"Sexing result: {sex_result['label']} ({sex_result['confidence']:.2f}), chamber count {chamber_count}."
        )

        if bool(sex_result.get("rejected", False)):
            reject_label = str(self.settings.reject_destination_label or "Reject")
            reject_reason = str(
                sex_result.get("rejection_reason", "")
                or sex_result.get("detail", "")
                or f"Rejected specimen with chamber count {chamber_count}."
            )
            self._update(selected_destination=reject_label, current_target=reject_label)
            ...
            self.hardware.drop_into_vial(
                float(self.settings.reject_destination_position_mm),
                vacuum_drop_delay_s=float(self.settings.vacuum_drop_delay_s),
                vacuum_release_settle_s=float(self.settings.vacuum_release_settle_s),
            )
            self.log(f"Specimen sent to {reject_label}: {reject_reason}")
            ...
            self._update(current_target="", selected_destination=reject_label)
            ...
            if finalize_stage:
                self._set_stage(
                    ...
                    message=f"{reject_reason} Capture the channel again for the next specimen.",
                )
    ...
    def start_test_sexing_snapshot(self) -> bool:
        def task():
            self.log("Capturing a sexing test snapshot.")
            self._clear_sexing_state("Capturing test snapshot...")
            sex_result = self.sexing.classify()
            self._set_sexing_state(sex_result)
            count = int(sex_result.get("count", 0) or 0)
            label = str(sex_result.get("label", "UNCERTAIN") or "UNCERTAIN")
            confidence = float(sex_result.get("confidence", 0.0) or 0.0)
            if bool(sex_result.get("rejected", False)):
                selected_destination = str(self.settings.reject_destination_label or "Reject")
            else:
                selected_destination = "Test only"
            self._update(selected_destination=selected_destination, current_target="")
            detail = str(sex_result.get("detail", "") or "Sexing test complete.")
            self.log(f"Sexing test result: {label} ({confidence:.2f}), chamber count {count}.")
            self._set_stage(
                WorkflowStage.READY,
                "Sexing test",
                next_action="Route Next Fly",
        ...
        return self._begin_task("test_sexing_snapshot", WorkflowStage.ROUTING, "Sexing test", "Capturing sexing test snapshot...", task)
