...
class App(tk.Tk):
    ...
    def _resolve_playback_source(self, kind: str) -> tuple[Path, bool]:
        ...
        if kind == "raw":
            ...
            processing_json = run_dir / "processed" / "processing_session.json"
            processing = load_json(processing_json) if processing_json.exists() else {}
            key = "annotated_video_path" if kind == "annotated" else "mask_video_path"
            candidates = []
            explicit = str(processing.get(key, "") or "").strip()
            if explicit:
