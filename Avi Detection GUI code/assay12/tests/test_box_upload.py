...
class TestDefaultBoxPaths(unittest.TestCase):
    def test_gui_and_templates_use_assayfinal_tokens_path(self):
        import box_upload
        self.assertEqual(str(box_upload.DEFAULT_BOX_DIR), '/home/team8/z.avi_assay_tests/assayfinal')
        self.assertEqual(box_upload.DEFAULT_BOX_TOKENS_PATH, '/home/team8/z.avi_assay_tests/assayfinal/box_tokens.json')

    def test_repo_root_detection_supports_flat_assayfinal_layout(self):
        import tempfile
        from pathlib import Path
        import box_upload

        with tempfile.TemporaryDirectory() as tmpdir:
            assayfinal = Path(tmpdir) / 'assayfinal'
            assayfinal.mkdir()
            legacy = assayfinal / 'box_login.py'
            legacy.write_text(
                "CLIENT_ID = 'abc'\n"
                "CLIENT_SECRET = 'def'\n"
                "BOX_PARENT_FOLDER_ID = '123'\n"
                "TOKENS_FILE = '/home/team8/z.avi_assay_tests/assayfinal/box_tokens.json'\n",
                encoding='utf-8',
            )
            original_file = box_upload.__file__
            try:
                box_upload.__file__ = str(assayfinal / 'box_upload.py')
                self.assertEqual(box_upload._repo_root_for_legacy(), assayfinal.resolve())
            finally:
                box_upload.__file__ = original_file
