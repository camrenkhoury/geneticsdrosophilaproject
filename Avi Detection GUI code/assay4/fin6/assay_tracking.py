...
def _estimate_component_reference_stats(
    stats: np.ndarray,
    *,
    min_area: int,
    max_area: int,
    ...
    for i in range(1, int(stats.shape[0])):
        _x, _y, w, h, area = [int(v) for v in stats[i].tolist()]
        if area < int(min_area) or area > int(max_area) or min(w, h) < 2:
            continue
        width = float(w)
        height = float(h)
        ...
        short_sides.append(max(1.0, min(width, height)))

    if not areas:
        return {}

    return {
        "median_area": float(np.median(np.asarray(areas, dtype=float))),
        "median_width": float(np.median(np.asarray(widths, dtype=float))),
        "median_height": float(np.median(np.asarray(heights, dtype=float))),
        "median_long_side": float(np.median(np.asarray(long_sides, dtype=float))),
        "median_short_side": float(np.median(np.asarray(short_sides, dtype=float))),
    }


def _split_parts_for_component(
    *,
    area: int,
    width: int,
    height: int,
    ...
    if int(area) > int(max_area):
        return max(2, int(round(float(area) / max(1.0, float(max_area)))))

    median_area = float(reference_stats.get("median_area", 0.0) or 0.0)
    median_long = float(reference_stats.get("median_long_side", 0.0) or 0.0)
    median_short = float(reference_stats.get("median_short_side", 0.0) or 0.0)
    if median_area <= 0.0 or median_long <= 0.0:
        return 1

    long_side = float(max(width, height))
    short_side = float(max(1, min(width, height)))
    area_ratio = float(area) / max(1.0, median_area)
    long_ratio = long_side / max(1.0, median_long)
    short_ratio = short_side / max(1.0, median_short)

    if area_ratio < 1.65 or long_ratio < 1.25:
        return 1
    if short_ratio < 0.75 and area_ratio < 2.20:
        return 1
    if float(area) < max(float(min_area) * 4.0, median_area * 1.45):
        return 1

    estimated_parts = int(round(max(area_ratio / 1.20, long_ratio / 1.15, short_ratio / 1.35)))
    return min(4, max(2, estimated_parts))
...
def _component_detections_from_mask(
    ...
    out: List[Detection] = []
    ox, oy = offset_xy
    reference_stats = _estimate_component_reference_stats(
        stats,
        min_area=min_area,
        max_area=max_area,
    )
    ...
    for i in range(1, int(count)):
        ...
        expected_parts = _split_parts_for_component(
            area=area,
            width=w,
            height=h,
            min_area=min_area,
...
def _build_compact_vial_report_df(vial_summary_df: pd.DataFrame) -> pd.DataFrame:
    if vial_summary_df.empty:
        return pd.DataFrame()

    report_cols: List[str] = []
    rename_map: Dict[str, str] = {}

    def add_column(name: Optional[str], label: str) -> None:
        if name is None or name in report_cols or name not in vial_summary_df.columns:
            return
        report_cols.append(name)
        rename_map[name] = label

    add_column("assay_tube_index", "Vial")
    add_column("configured_flies_per_vial", "Configured Flies")
    add_column(_first_present_column(vial_summary_df, ["number_of_flies_detected", "n_tracks"]), "Tracked Flies")
    add_column("unique_threshold_crossings", "Threshold Crossings")
    add_column("fraction_crossing_by_10s", "Crossing Fraction")

    distance_col = _first_present_column(
        vial_summary_df,
        [
            "estimated_average_distance_travelled_mm",
            "estimated_average_distance_travelled_px",
        ],
    )
    if distance_col is not None:
        unit = "mm" if distance_col.endswith("_mm") else "px"
        add_column(distance_col, f"Avg Distance ({unit})")

    velocity_col = _first_present_column(
        vial_summary_df,
        [
            "estimated_mean_velocity_mm_s",
            "estimated_mean_velocity_px_s",
            "cohort_velocity_mm_s",
            "cohort_velocity_px_s",
        ],
    )
    if velocity_col is not None:
        unit = "mm/s" if velocity_col.endswith("_mm_s") else "px/s"
        add_column(velocity_col, f"Estimated Velocity ({unit})")

    peak_velocity_col = _first_present_column(
        vial_summary_df,
        [
            "peak_estimated_velocity_mm_s",
            "peak_estimated_velocity_px_s",
        ],
    )
    if peak_velocity_col is not None:
        unit = "mm/s" if peak_velocity_col.endswith("_mm_s") else "px/s"
        add_column(peak_velocity_col, f"Peak Velocity ({unit})")

    add_column("first_crossing_time_s", "First Crossing (s)")

    display_df = vial_summary_df[report_cols].copy().rename(columns=rename_map)
    for col in display_df.columns:
        if pd.api.types.is_float_dtype(display_df[col]):
            display_df[col] = display_df[col].map(lambda value: "" if pd.isna(value) else round(float(value), 2))
        elif pd.api.types.is_integer_dtype(display_df[col]):
            display_df[col] = display_df[col].map(lambda value: "" if pd.isna(value) else int(value))
    return display_df
...
def _render_summary_bar_chart(
    ...
    *,
    ...
    if vial_summary_df.empty or value_col not in vial_summary_df.columns or "assay_tube_index" not in vial_summary_df.columns:
        return False
    chart_df = vial_summary_df[["assay_tube_index", value_col]].copy()
    chart_df[value_col] = pd.to_numeric(chart_df[value_col], errors="coerce")
    chart_df = chart_df.dropna(subset=[value_col]).sort_values("assay_tube_index")
    if chart_df.empty:
        return False

    labels = [f"Vial {int(v)}" for v in chart_df["assay_tube_index"].tolist()]
    values = chart_df[value_col].astype(float).tolist()
    fig = plt.figure(figsize=(10.5, 5.4))
    ax = fig.add_subplot(111)
    ax.set_title(title)
    ax.bar(range(len(values)), values, color="#5b7c99")
...
def generate_graphs_and_pdf(
    ...
    with PdfPages(pdf_path) as pdf:
        cover_lines = [
            ...
            f"Duration (s): {session_meta.get('seconds') or session_meta.get('processed_duration_s') or session_meta.get('assay_duration_s', '')}",
            f"Frames processed: {session_meta.get('frames_processed', '')}",
            f"Tracking mode: {session_meta.get('tracking_mode', '')}",
            f"Configured max flies/vial: {session_meta.get('max_flies_per_vial', '')}",
            f"Unique threshold crossings total: {session_meta.get('unique_threshold_crossings_total', '')}",
            "Summary tables cap tracked-fly and crossing totals to the configured flies-per-vial limit.",
        ]
        _render_pdf_text_page(pdf, "Fruit fly assay report", cover_lines, fontsize=11)

        compact_vial_df = _build_compact_vial_report_df(vial_summary_df)
        if not compact_vial_df.empty:
            _render_pdf_table_pages(pdf, "Per-vial summary", compact_vial_df, max_rows=18, max_cols=8)
        ...
        if distance_col is not None:
            distance_unit = "mm" if distance_col.endswith("_mm") else "px"
            distance_path = graphs_dir / "distance_overview.png"
            if _render_summary_bar_chart(
                pdf,
                distance_path,
                vial_summary_df,
                value_col=distance_col,
                title="Average distance travelled by vial",
                ylabel=f"Average distance travelled ({distance_unit})",
            ):
                paths["distance_plot_png"] = str(distance_path)
        ...
        if velocity_col is not None:
            velocity_unit = "mm/s" if velocity_col.endswith("_mm_s") else "px/s"
            velocity_path = graphs_dir / "velocity_plot.png"
            if _render_summary_bar_chart(
                pdf,
                velocity_path,
                vial_summary_df,
                value_col=velocity_col,
                title="Estimated velocity by vial",
                ylabel=f"Estimated velocity ({velocity_unit})",
            ):
