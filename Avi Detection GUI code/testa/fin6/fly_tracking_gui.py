...
from box_upload import should_auto_upload, write_box_templates
...
class App(tk.Tk):
    ...
    def _build_vars(self) -> None:
        ...
        default_box_dir = Path.home() / ".config" / "fin6"
        self.box_tokens_file_var = tk.StringVar(value=str(default_box_dir / "box_tokens.json"))
        self.box_config_file_var = tk.StringVar(value=str(default_box_dir / "box_config.json"))
    ...
    def _default_box_paths(self) -> tuple[Path, Path]:
        box_dir = Path.home() / ".config" / "fin6"
        return box_dir / "box_config.json", box_dir / "box_tokens.json"
    ...
    def write_box_templates(self) -> None:
        ...
        self._set_footer("Box template files are ready.")
        messagebox.showinfo(
            "Box template files",
    ...
    def _default_profile(self, name: str = "default") -> AssayProfile:
        ...
        profile.box_upload.config_file = str(box_config_path)
    ...
    def load_profile(self, profile: AssayProfile) -> None:
        ...
        self.box_config_file_var.set(profile.box_upload.config_file or str(box_config_path))
    ...
    def _build_profile_from_vars(self) -> AssayProfile:
        ...
        profile.box_upload.config_file = self.box_config_file_var.get().strip() or str(box_config_path)
