...
class _NullMotorBackend:
    def __init__(self, settings: MotorSettings, *, reason: str = ""):
    ...
    def pulse(self) -> None:
        if self.settings.enabled:
            message = (
                ...
                "Install gpiozero or RPi.GPIO, or ensure the repository vibration module can be imported."
            )
            if self.reason:
                message += f" Last backend error: {self.reason}"
...
class _ExternalModuleBackend:
    def __init__(self, settings: MotorSettings):
        ...
        if not _external_module_is_usable(self.module):
            raise MotorError(
                f"Module '{module_name}' is present, but it is running in mock mode because GPIO support "
                "is unavailable in this Python environment."
            )
...
def _external_module_is_usable(module: Any) -> bool:
    gpio_available = getattr(module, "GPIO_AVAILABLE", None)
    ...
    for module_name in _candidate_module_names(settings):
        ...
        try:
            if module_name in _MODULE_CACHE:
                return _MODULE_CACHE[module_name], module_name
    ...
    def _build_backend(self, settings: MotorSettings):
        ...
        for backend_cls in backends:
            try:
                ...
                last_error = str(exc or backend_cls.__name__)
                continue
        return _NullMotorBackend(settings, reason=last_error)
