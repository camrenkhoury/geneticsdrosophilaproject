...
from motor_control import MotorError, MotorSettings, VibrationMotor
...
class MotorControlTests(unittest.TestCase):
    ...
    def test_mocked_external_module_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            module_path = root / "vibration.py"
            module_path.write_text(
            ...
            try:
                ...
                with self.assertRaises(MotorError) as ctx:
                    VibrationMotor(MotorSettings(enabled=True, pulse_ms=1, settle_delay_ms=0, backend="module")).test()
                self.assertIn("mock mode", str(ctx.exception))
            finally:
                if old_path is None:
                    os.environ.pop("FIN6_VIBRATION_PATH", None)
                else:
