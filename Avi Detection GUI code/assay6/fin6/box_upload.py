...
DEFAULT_BOX_CONFIG_TEMPLATE: Dict[str, Any] = {
    "_instructions": [
        ...
        "This template stays disabled until you explicitly enable Box upload in the fin6 profile.",
    ],
    "enabled": False,
    "upload_after_processing": False,
    "upload_after_recording": False,
...
def _resolve_legacy_token_path(raw_value: str, *, relative_to: Path) -> Path:
    ...
    if not candidate.is_absolute():
        candidate = Path(os.path.abspath(str(relative_to / candidate)))
    else:
...
def _rebase_path_under_anchor(path: str | Path, anchor: str | Path) -> Path:
    candidate = _absolute_local_path(path)
    anchor_path = _absolute_local_path(anchor)
    try:
        candidate.relative_to(anchor_path)
        return candidate
    except Exception:
        pass
    try:
        rel = candidate.resolve().relative_to(anchor_path.resolve())
    except Exception:
        return candidate
    rebased = anchor_path / rel
    return rebased if rebased.exists() else candidate


def _relative_to_anchor(path: Path, anchor: Path) -> Path:
    try:
        return path.relative_to(anchor)
    except ValueError:
        return path.resolve().relative_to(anchor.resolve())
    ...
    *,
    ...
    if legacy_cfg:
        config_payload["_instructions"] = [
            ...
            "Uploads stay disabled until you enable them in the fin6 profile.",
        ]
...
def _is_snapshot_artifact(path: Path) -> bool:
    parts = {part.lower() for part in path.parts}
    name = path.name.lower()
    return "snapshots" in parts or name.startswith("snapshot_") or name == "snapshot_index.csv"
...
def _resolve_processing_dir_candidate(candidate: str | Path | None, run_dir: Path) -> Optional[Path]:
    ...
    if not path.is_absolute():
        path = _absolute_local_path(run_dir / path)
    else:
        path = _absolute_local_path(path)
    path = _rebase_path_under_anchor(path, run_dir)
...
def _resolve_raw_video_path(run_dir: Path) -> Optional[Path]:
    ...
    if raw_candidate:
        ...
        if not raw_path.is_absolute():
            raw_path = _absolute_local_path(run_dir / raw_path)
        else:
            raw_path = _absolute_local_path(raw_path)
        raw_path = _rebase_path_under_anchor(raw_path, run_dir)
...
def collect_artifacts(run_dir: str | Path, mode: str = "summaries", *, include_backgrounds: bool = False) -> List[Path]:
    ...
    if mode_norm in {"full", "full_session", "full-folder", "full_folder"}:
        return sorted(_rebase_path_under_anchor(path, run_dir) for path in files)
    ...
    if mode_norm in {"summaries", "summaries+videos", "summaries_videos", "videos", "summary+videos"}:
        selected = [path for path in selected if not _is_snapshot_artifact(path)]
    return sorted(_rebase_path_under_anchor(path, run_dir) for path in selected)
    ...
    *,
    ...
    try:
        ...
        for file_path in files:
            rel_path = _relative_to_anchor(file_path, run_dir)
