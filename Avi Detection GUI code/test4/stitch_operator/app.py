...
class OperatorApp(tk.Tk):
    ...
    def _image_panel(self, parent, key: str, title: str, *, width: int = 520, height: int = 300) -> tk.Frame:
        ...
        panel = tk.Label(body, text="No preview available", bg="#000000", fg="#d0d0d0", font=BODY_FONT, width=width // 10, height=height // 20)
        panel.pack(fill="both", expand=True)
        self._image_widgets[key] = {"label": panel, "photo": None, "size": (width, height), "last_path": "", "last_mtime": None}
        return outer
    ...
    def _update_image_widget(self, key: str, path_text: str, placeholder: str) -> None:
        ...
        try:
            ...
            if target.get("last_path") == str(path) and target.get("last_mtime") == mtime and target.get("photo") is not None:
                return
            image = Image.open(path).convert("RGB")
            max_w, max_h = target["size"]  # type: ignore[misc]
    ...
    def _build_workflow_page(self, parent) -> tk.Frame:
        ...
        hero, body = self._card(page, "Guided workflow", "The main operator surface stays calm and minimal. Dense controls are contained under Debug / Advanced.")
        ...
        self._metric_block(body, "Current stage", self.workflow_stage_value).grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        self._metric_block(body, "Operator guidance", self.workflow_message_value).grid(row=0, column=1, sticky="nsew", padx=6)
        self._metric_block(body, "Next action", self.workflow_next_value).grid(row=0, column=2, sticky="nsew", padx=(12, 0))

        actions = tk.Frame(body, bg=PALETTE.surface_low)
        actions.grid(row=1, column=0, columnspan=3, sticky="ew", pady=(16, 0))
        self._action_button(actions, "Initialize", lambda: self.controller.start_initialize()).pack(side="left", padx=(0, 10))
        self._action_button(actions, "Capture Channel", lambda: self.controller.start_capture_channel()).pack(side="left", padx=10)
        self._action_button(actions, "Route Next Fly", lambda: self.controller.start_route_next_fly()).pack(side="left", padx=10)
        self._action_button(actions, "Run Assay", lambda: self.controller.start_run_assay()).pack(side="left", padx=10)
        self._action_button(actions, "Process Last", lambda: self.controller.start_process_last_assay(), primary=False).pack(side="left", padx=10)
        self._action_button(actions, "Upload Last", lambda: self.controller.start_upload_last_run(), primary=False).pack(side="left", padx=10)
        ...
        self._image_panel(left, "workflow_channel", "Channel view", width=640, height=330).grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        self._image_panel(left, "workflow_sexing", "Sexing preview", width=420, height=330).grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        vial_section, vial_body = self._card(left, "Assay destinations", "Counts stay visible, but dense diagnostics remain in Debug.")
        vial_section.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(18, 0))
        ...
        for title, var in [
            ...
            tk.Label(row, textvariable=var, bg=PALETTE.surface_low, fg=PALETTE.text, font=BODY_FONT, anchor="w", wraplength=300, justify="left").pack(side="left", fill="x", expand=True)

        hint_card, hint_body = self._card(right, "Operator notes")
        hint_card.pack(fill="both", expand=True, pady=(18, 0))
        self.workflow_hint_var = tk.StringVar(value="Initialize, then capture the channel.")
        tk.Label(hint_body, textvariable=self.workflow_hint_var, bg=PALETTE.surface_low, fg=PALETTE.text_muted, font=BODY_FONT, justify="left", wraplength=360).pack(anchor="w")
        return page
    ...
    def _build_channel_page(self, parent) -> tk.Frame:
        ...
        self._image_panel(page, "channel_main", "Channel capture", width=860, height=560).grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        right = tk.Frame(page, bg=PALETTE.background)
        right.grid(row=0, column=1, sticky="nsew")

        setup_card, setup_body = self._card(right, "Channel setup", "These are necessary operator actions, but the main workflow stays focused on the next step.")
    ...
    def _build_loading_page(self, parent) -> tk.Frame:
        ...
        self._image_panel(left, "loading_sexing", "Sexing chamber", width=760, height=520).pack(fill="both", expand=True)

        right = tk.Frame(page, bg=PALETTE.background)
        right.grid(row=0, column=1, sticky="nsew")
        top_card, top_body = self._card(right, "Loading / sexing", "The operator sees only the current classification result and the next destination.")
    ...
    def _build_assay_page(self, parent) -> tk.Frame:
        ...
        left = tk.Frame(page, bg=PALETTE.background)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        self._image_panel(left, "assay_main", "Assay preview", width=860, height=560).pack(fill="both", expand=True)

        right = tk.Frame(page, bg=PALETTE.background)
        right.grid(row=0, column=1, sticky="nsew")
        setup_card, setup_body = self._card(right, "Assay operations", "Profiles, background, calibration, recording, processing, and upload are all wired to the real fin6 workflow.")
        setup_card.pack(fill="x")
    ...
    def _build_results_page(self, parent) -> tk.Frame:
        ...
        left = tk.Frame(page, bg=PALETTE.background)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        self._image_panel(left, "results_preview", "Last assay / processed preview", width=860, height=520).pack(fill="both", expand=True)

        right = tk.Frame(page, bg=PALETTE.background)
        right.grid(row=0, column=1, sticky="nsew")
        summary_card, summary_body = self._card(right, "Results summary", "PDF/report generation and optional Box upload are driven by the existing fin6 processing pipeline.")
    ...
    def _build_debug_page(self, parent) -> tk.Frame:
        ...
        note_card, note_body = self._card(body, "Debug / Advanced", "This is the containment zone for dense controls, manual recovery, paths, logs, and raw state.")
        note_card.grid(row=0, column=0, sticky="ew", pady=(0, 18))
        tk.Label(note_body, text="Use these controls for troubleshooting, validation, calibration recovery, and direct hardware actions.", bg=PALETTE.surface_low, fg=PALETTE.text_muted, font=BODY_FONT).pack(anchor="w")
        ...
        model_card, model_body = self._card(body, "Models + paths")
        ...
        profile_card, profile_body = self._card(body, "Assay profiles")
        profile_card.grid(row=4, column=0, sticky="ew", pady=(0, 18))
        combo_row = tk.Frame(profile_body, bg=PALETTE.surface_low)
        ...
        state_card.grid(row=5, column=0, sticky="nsew", pady=(0, 18))
    ...
    def _refresh_status(self, snapshot: OperatorState):
        ...
        if snapshot.brief_error:
            ...
            hint = "The channel result is stale after routing. Capture the channel again before the next pick."
        elif snapshot.stage == WorkflowStage.CHANNEL and snapshot.channel.fly_remaining:
            hint = "Route the next fly, then capture the channel again once the nozzle returns."
    ...
    def _refresh_images(self, snapshot: OperatorState):
        self._update_image_widget("workflow_channel", snapshot.channel.annotated_image_path, "Capture the channel to preview detections.")
        self._update_image_widget("workflow_sexing", snapshot.sexing.image_path, "Route a fly to view the sexing preview.")
        self._update_image_widget("channel_main", snapshot.channel.annotated_image_path or snapshot.channel.raw_image_path, "No channel preview available.")
        self._update_image_widget("loading_sexing", snapshot.sexing.image_path, "No sexing preview available.")
        assay_preview = snapshot.assay.preview_image_path or snapshot.assay.pdf_path
        self._update_image_widget("assay_main", snapshot.assay.preview_image_path, "Capture an assay preview or run an assay.")
        self._update_image_widget("results_preview", snapshot.assay.preview_image_path, "Process an assay to populate results previews.")
