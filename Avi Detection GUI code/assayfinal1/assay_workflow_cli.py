...
from box_upload import DEFAULT_BOX_DIR, write_box_templates
...
def command_upload(args: argparse.Namespace) -> int:
    ...
    result = manual_upload_run(run_dir, profile.box_upload, artifact_mode=args.artifact_mode or None, logger=_logger)
    print(f"Uploaded artifacts for: {run_dir}")
    print(result)
    return 0

def command_box_write_template(args: argparse.Namespace) -> int:
    target_dir = Path(args.output_dir).expanduser() if args.output_dir else DEFAULT_BOX_DIR
    result = write_box_templates(target_dir, overwrite=bool(args.overwrite))
    print("Wrote Box template files:")
    print(f"- config: {result['config_file']}")
    print(f"- tokens: {result['tokens_file']}")
    print(f"- env: {result['env_file']}")
    if result.get("seeded_from"):
        print(f"- seeded from legacy repo Box settings: {result['seeded_from']}")
        print(f"- tokens setup: {result.get('tokens_write_mode', 'written')}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = subparsers.add_parser("box-write-template", help="Write Box config files, auto-seeding from legacy capture.py Box settings when available")
    p.add_argument("--output-dir", default="", help="Directory for box_config.json and box_tokens.json")
    p.add_argument("--overwrite", action="store_true", help="Overwrite existing template files")
    p.set_defaults(func=command_box_write_template)
