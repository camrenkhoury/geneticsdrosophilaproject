...
class WorkflowController:
    ...
    def _merge_sexing_payloads(
        ...
        *,
        ...
        merged = dict(secondary or primary)
        errors: List[str] = []
        ...
        if "count" not in merged:
            merged["count"] = self._sex_result_count(secondary or primary)
        return merged
    ...
    def _redrop_for_resex(self, *, fly_index: int) -> None:
        self._update(
            current_target=f"Fly {fly_index}: re-sexing",
            selected_destination="",
            status_message=f"Fly {fly_index}: re-checking sex with a second chamber pass.",
        )
        self.log(f"Fly {fly_index}: re-acquiring specimen for sex verification.")
    ...
    def _auto_route_from_source_impl(
        ...
        *,
        ...
        first_label = self._sex_result_label(first_pass)
        first_count = self._sex_result_count(first_pass)
        first_detail = str(first_pass.get("detail", "") or "").strip()

        second_pass: Optional[Dict[str, Any]] = None
        second_detail = ""
        destination: Optional[VialState] = None
        destination_mode = "junk"
        route_from_chamber = False
        ...
        if self._sex_result_is_confirmed_candidate(first_pass):
            self._redrop_for_resex(fly_index=fly_index)
            second_pass = self._classify_sexing_pass(fly_index=fly_index, pass_index=2)
            second_label = self._sex_result_label(second_pass)
            second_count = self._sex_result_count(second_pass)
            second_detail = str(second_pass.get("detail", "") or "").strip()
            combined_confidence = min(
                float(first_pass.get("confidence", 0.0) or 0.0),
                float(second_pass.get("confidence", 0.0) or 0.0),
            )

            if self._sex_result_is_confirmed_candidate(second_pass) and second_label == first_label:
                destination, destination_mode = self.choose_auto_destination(second_label)
                route_from_chamber = True
                outcome = "routed"
                if destination_mode == "junk":
                    detail = (
                        f"{first_detail} || {second_detail} || "
                        f"Confirmed {second_label} on both passes, but the {second_label} vials are full. "
                        f"Sending the specimen to {destination.label}."
                    )
                else:
                    detail = (
                        f"{first_detail} || {second_detail} || "
                        f"Confirmed {second_label} on both passes."
                    )
                final_sex_result = self._merge_sexing_payloads(
                    first_pass,
                    second_pass,
                    detail=detail,
                    force_label=second_label,
                    force_uncertain=False,
                    force_confidence=combined_confidence,
                )
            else:
                destination = self.choose_junk_destination()
                destination_mode = "junk"
                route_from_chamber = self._sex_result_has_specimen(second_pass)
                mismatch_reason = "Sex verification failed"
                if second_count <= 0:
                    mismatch_reason = "Second sex pass did not isolate a fly"
                elif second_count > 1:
                    mismatch_reason = f"Second sex pass found multiple flies (count={second_count})"
                elif second_label not in {"male", "female"}:
                    mismatch_reason = f"Second sex pass returned {second_label.upper()}"
                elif second_label != first_label:
                    mismatch_reason = (
                        f"Sex mismatch between passes ({first_label.upper()} then {second_label.upper()})"
                    )
                reject_reason = mismatch_reason
                detail = (
                    f"{first_detail} || {second_detail} || {reject_reason}. "
                    + (
                        f"Sending the specimen to {destination.label}."
                ...
                if second_count <= 0:
                    ...
                    extra_errors.append(f"REJECT_COUNT_{second_count}")
                elif second_label not in {"male", "female"}:
                    extra_errors.append("REJECT_UNCERTAIN")
                if second_label != first_label:
                    extra_errors.append(f"SEX_MISMATCH:{first_label}->{second_label}")
                final_sex_result = self._merge_sexing_payloads(
                    first_pass,
                    second_pass,
                    detail=detail,
            ...
            destination = self.choose_junk_destination()
            destination_mode = "junk"
            route_from_chamber = self._sex_result_has_specimen(first_pass)
            if first_count <= 0:
                reject_reason = f"Rejected after first sex pass because count={first_count}"
            elif first_count > 1:
                reject_reason = f"Rejected after first sex pass because multiple flies were detected (count={first_count})"
            else:
                reject_reason = f"Rejected after first sex pass because the classifier returned {first_label.upper()}"
            detail = (
                f"{first_detail} || {reject_reason}. "
                + (
                    f"Sending the specimen to {destination.label}."
                    if route_from_chamber
                    else "No specimen remained in the chamber; verifying against the channel."
            ...
            if first_count <= 0:
                ...
                extra_errors.append(f"REJECT_COUNT_{first_count}")
            else:
                extra_errors.append("REJECT_UNCERTAIN")
            final_sex_result = self._merge_sexing_payloads(
                first_pass,
        ...
        if (
            ...
            and second_pass is None
            and first_count == 1
            and first_label not in {"male", "female"}
        ):
            self._update(
                selected_destination="",
                current_target=f"Fly {fly_index}: verifying uncertain pickup",
                status_message=(
                    f"Fly {fly_index}: uncertain first sex pass. Refreshing the channel before routing to {destination.label}."
                ),
            )
        ...
        if outcome == "routed":
            ...
            if destination_mode == "junk":
                message = (
                    f"Fly {fly_index}: confirmed {self._sex_result_label(final_sex_result).upper()} twice, "
