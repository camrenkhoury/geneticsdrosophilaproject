...
class OperatorApp(tk.Tk):
    ...
    def _build_assay_page(self, parent) -> tk.Frame:
        ...
        self._button_grid(
            setup_body,
            [
                ("Capture Preview", lambda: self.controller.start_capture_assay_preview(), True),
                ("Capture Background", lambda: self.controller.start_capture_assay_background(), False),
                ("Restore Previous", lambda: self.controller.start_restore_assay_background(), False),
                ("Calibrate", lambda: self.controller.start_calibrate_assay(), False),
                ("Run Assay", lambda: self.controller.start_run_assay(), True),
                ("Process Last", lambda: self.controller.start_process_last_assay(), False),
                ("Upload Last", lambda: self.controller.start_upload_last_run(), False),
            ],
            columns=2,
            width=None,
        )
    ...
    def _build_results_page(self, parent) -> tk.Frame:
        ...
        self._button_grid(
            summary_body,
            [
                ("Open Run Folder", self._open_last_run, False),
                ("Open Processed Folder", self._open_processed_dir, False),
                ("Open Summary CSV", self._open_summary_csv, False),
                ("Open PDF", self._open_last_pdf, False),
            ],
            columns=2,
            width=None,
        )
    ...
    def _refresh_status(self, snapshot: OperatorState):
        ...
        detail_text = snapshot.sexing.detail or "Route the next fly to refresh the sexing result."
        if detail_text:
