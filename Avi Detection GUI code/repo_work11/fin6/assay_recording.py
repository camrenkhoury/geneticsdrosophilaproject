...
from assay_tracking import ensure_calibration_matches_shape
...
def validate_recording_requirements(profile: AssayProfile, project_root: str | Path) -> Dict[str, Path]:
    ...
    background_bgr = cv2.imread(str(store.current_transformed_path), cv2.IMREAD_COLOR)
    if background_bgr is None:
        raise RecordingError(f"Could not read transformed background image: {store.current_transformed_path}")
    calibration_resized = False
    calibration_backup_path: Optional[Path] = None
    try:
        _calibration, calibration_resized, calibration_backup_path = ensure_calibration_matches_shape(
            calibration_path,
            [int(background_bgr.shape[0]), int(background_bgr.shape[1])],
            save_backup=True,
        )
    except Exception as exc:
        raise RecordingError(
            "Calibration dimensions do not match the transformed background and could not be repaired automatically. "
            "Re-save the calibration on the current transformed background, or capture/import a matching background."
        ) from exc
    ...
    return {
        ...
        "calibration_resized": bool(calibration_resized),
        "calibration_backup_path": calibration_backup_path,
    }
...
def record_assay_run(
    ...
    *,
        ...
        with open_assay_camera(
            ...
            try:
                with VibrationMotor(profile.motor) as motor:
                    ...
                    motor.pulse()
                    if profile.motor.enabled and hasattr(camera, 'reopen') and callable(getattr(camera, 'reopen')):
                        logger("Reopening assay camera after motor pulse to recover from transient USB dropouts...")
                        camera.reopen()
                        camera_index_in_use = getattr(camera, "camera_index_in_use", camera_index_in_use)
            except MotorError:
                raise
            except Exception as exc:
    ...
    manifest = {
        ...
        "calibration_resized": bool(required.get("calibration_resized", False)),
        "calibration_backup_path": None if required.get("calibration_backup_path") is None else str(Path(required["calibration_backup_path"]).resolve()),
